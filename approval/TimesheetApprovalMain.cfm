...
<body>
    ...
    <cfquery name="tmonthselect" datasource="#dts#">
        SELECT tm.id, tm.placementno, tm.pdate, tm.tsrowcount, tm.status, tm.tmonth, min(tm.pdate) AS first, max(tm.pdate) AS last, tm.updated_on, tm.mgmtremarks, 
        tm.tyear, (SELECT empname FROM #dts2#.placement WHERE placementno = tm.placementno) AS empname
        FROM timesheet tm
        WHERE tm.placementno IN 
        (SELECT placementno FROM #dts2#.placement WHERE hrmgr = '#getHQstatus.entryid#')
        GROUP BY tm.tmonth, tm.tyear, tm.placementno;
    </cfquery>

...
<cfquery name="getposlist" datasource="#dts2#">
	<!---SELECT * FROM (select * from placement where year(completedate) >=#company_details.myear#) a 
    INNER JOIN #dts#.timesheet b on a.placementno = b.placementno 
    WHERE (b.status='Submitted For Approval' or b.status='Submitted For Approval 2') 
    and a.hrMgr = "#getHQstatus.entryid#"
    GROUP BY b.empno, a.placementno
    ORDER BY b.empno--->
    SELECT b.id, a.empname, b.placementno, b.tmonth
    FROM placement a
    LEFT JOIN #dts#.timesheet b on a.placementno = b.placementno
    WHERE a.hrmgr = "#getHQstatus.entryid#"
    AND year(a.completedate) >= "#company_details.myear#"
    AND b.status IN ("Submitted For Approval")
    AND b.tsrowcount = 0
    <!---AND year(b.pdate) = "#company_details.myear#"--->
    GROUP BY b.placementno, b.empno, b.pdate
    ORDER by b.empno    
</cfquery>
...
<cfloop query="getposlist">
    ...
    <cfquery name="getTimesheet" datasource="#dts#">
        SELECT min(pdate) AS first, max(pdate) AS last, tmonth, tsrowcount, updated_on, mgmtremarks
        FROM timesheet 
        WHERE placementno = "#getposlist.placementno#"
        AND tmonth = "#getposlist.tmonth#"
        AND status IN ("Submitted For Approval")
        <!---AND year(pdate) = "#company_details.myear#"--->
    </cfquery>
    ...
    --->
        <tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='f3bd90';">
            ...
            <td align="center" valign="top">#dateformat(getTimesheet.updated_on,'dd/mm/yyyy')#</td>
            <td align="center" valign="top">#MonthAsString(getTimesheet.tmonth)#</td>	<!---<cftry>#monthasstring(month(endingDate))#<cfcatch>#getListofSubmit.tmonth#</cfcatch></cftry>--->	<!---changed tmonth to end date month--->
            <td align="center" valign="top">#dateformat(getTimesheet.first,'dd/mm/yyyy')#</td>
            <td align="center" valign="top">#dateformat(getTimesheet.last,'dd/mm/yyyy')#</td>
            <td align="center" valign="top"><a href="TimesheetApprovalView.cfm?pno=#getposlist.placementno#&datestart=#dateformat(getTimesheet.first,'yyyy-mm-dd')#&dateend=#dateformat(getTimesheet.last,'yyyy-mm-dd')#&tmonth=#getTimesheet.tmonth#" target="_blank">View</a></td>
        ...
        <table>
            <tr>
                ...
                <td valign="top">
                    ...
                    <a href="##" onclick="confirmApprove#apptype#('app','#getTimesheet.tmonth#','#getposlist.placementno#','#dateformat(getTimesheet.first, 'yyyy-mm-dd')#','#dateformat(getTimesheet.last, 'yyyy-mm-dd')#', '#submitRow#')"><img height="30px" width="30px" src="/images/1.png" alt="Approve" border="0"><br/>Approve</a>
                    <!---added--->
                </td>
                <td valign="top">
                    <a href="##" onclick="if(document.getElementById('management_#submitRow#').value == ''){alert('Please fill in the reason in Mgmt Remarks to cancel')}else{confirmDecline('dec','#getTimesheet.tmonth#','#getposlist.placementno#','#dateformat(getTimesheet.first, 'yyyy-mm-dd')#','#dateformat(getTimesheet.last, 'yyyy-mm-dd')#', '#submitRow#')}"><img height="30px" width="30px" src="/images/2.png" alt="Decline" border="0"><br />Decline</a>
                </td>
				<td></td>            
            </tr>
        </table>

        <input type="hidden" name="looprem" id="looprem" value="#getTimesheet.tmonth#">
...
<div class="tabbertab">
    ...
    <cfquery name="getposlist" datasource="#dts2#">
        ...
        ORDER BY b.empno--->
        <!---SELECT * FROM (select * from placement where year(completedate) >=#company_details.myear#-1) a 
        INNER JOIN #dts#.timesheet b on a.placementno = b.placementno 
	...
	<cfinput type="hidden" name="rows" id="rows" value="#getposlist.CurrentRow#">
	    <cfloop query="getposlist">
            ...
            <tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='f3bd90';">
                ...
                <td align="center">#dateformat(getTimesheet.updated_on,'dd/mm/yyyy')#</td>     
                <!---changed--->      
                <td align="center">#MonthAsString(getTimesheet.tmonth)#</td>
                
                <td align="center">#dateformat(getTimesheet.first,'dd/mm/yyyy')#</td>
                <td align="center">#dateformat(getTimesheet.last,'dd/mm/yyyy')#</td>
                <td align="center"><a href="TimesheetApprovalView.cfm?pno=#getposlist.placementno#&datestart=#dateformat(getTimesheet.first,'yyyy-mm-dd')#&dateend=#dateformat(getTimesheet.last,'yyyy-mm-dd')#&tmonth=#getTimesheet.tmonth#" target="_blank">View</a></td>
              ...
              <!---   <td align="center"> <cfif getposlist.signdoc neq ''><a href="#getposlist.signdoc#" target="_blank">View</a></cfif></td> --->
                <td align ="left">#getTimesheet.mgmtremarks#
            </tr>
            <cfset submitRow += 1>
            <!---</cfloop>--->
		</cfloop>

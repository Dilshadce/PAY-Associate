...
<cfquery name="company_details" datasource="payroll_main">
   SELECT * FROM gsetup WHERE comp_id = <cfqueryparam cfsqltype="cf_sql_varchar" value="#HcomID#">
        ...
        SELECT * FROM timesheet WHERE placementno = '#pno#' and pdate between "#datestart#" and "#dateend#"
        </cfquery>
...
<tr height=20 style='height:15.0pt'>
  ...
  height:15.0pt'><cfif month(datestart) eq month(dateend)>#dateformat(datestart,'Mmm yyyy')#<cfelse>#dateformat(datestart,'Mmm yyyy')# to #dateformat(dateend,'Mmm yyyy')#</cfif></td>

...
<cffunction name="listLoggingHistory" access="remote" returntype="struct">
	...
	Select * from hmusers where userId = '#GetAuthUser()#'
	</cfquery>
	...
	<cfquery name="getFilteredDataSet" datasource="payroll_main">
		...
		FROM userlog AS a
		LEFT JOIN hmusers AS b
		ON a.userLogID = b.userID
		WHERE a.udatabase = "#left(dts,len(dts)-2)#"
		AND b.userGrpID != 'super'
		AND b.entryid = "#getHQstatus.entryid#"
        AND LEFT(a.userLogID,5) != 'ultra'
		#sWhere#
		#sOrder#
		#sLimit#
<!---        

 ...
 <cfreturn nocal/lencontrol>
		...
		<cffunction name="updatePay" access="public" returntype="any">
            ...
            <cfargument name="weekpay" required="yes">
			...
			<cfif OTRATETYPE neq  "C">
				...
				<cfloop from="1" to="6" index="i">
					...
					<cfloop query="select_aw_OT">
						...
						<cfif getAwFromweekpay.AW neq 0 >
							<cfset awot = awot + getAwFromweekpay.AW>
						</cfif>
					</cfloop>
            ...
            <cfset weeksdedamt = 0>
	  		...
	  		<cfloop from="1" to="15" index="i">
		       ...
		       <!---  <cfset deddata = #evaluate("select_empdata.#dedvar1#")#> --->
				...
				<cfset PAY_TIMES = 7>
                <cfset tempgross = grosspay> <!--- temp keep grosspay to calculate community fund --->
				<cfif DED gt 0 and new_ded_formula neq "" and ggrosspay gt 0 and ded_type eq "F">
	        		<cfif use_ded_formula neq 0 >
                    
                    <!--- Calculate week community fund correctly   ded109 to ded 115 --->
<!--- 					<cfif left(db,3) eq "gaf">	 --->	
                    <cfif i gte 9>
                    
						<cfset looptable = right(weekpay,1)-1> 
                        <cfquery name="getcded2" datasource="#db#">
                            SELECT sum(#dedvar_update#) as weeksdedamt,sum(grosspay) as grosspay FROM (
                            SELECT #dedvar_update#,payyes,empno,grosspay FROM payweek1
                        <cfloop from="2" to="#looptable#" index="j">
                            UNION ALL SELECT #dedvar_update#, payyes,empno,grosspay from payweek#j# 
                        </cfloop>
                            )as a
                            WHERE
                            payyes = "Y" AND empno = "#empno#"
                        </cfquery>
						<cfset grosspay = grosspay + val(getcded2.grosspay)>
                        <cfset weeksdedamt = val(getcded2.weeksdedamt)>
...
<!---                     </cfif>  --->
				        ...
				        <!--- deduct previous month fund --->
						<cfset new_value = #numberformat(new_value-val(weeksdedamt),'.__')# >

		        	<cfelse>
		        		<cfset new_value = DED>
		        	</cfif>
		           	
                    <cfset grosspay = tempgross>	<!--- assign original grosspay back --->
                    
					<!--- end of week community fund --->
	        ...
	        <cfif epfno neq "" and epfcat neq "X" and fix_cpf eq 0 and epf_selected neq 0>
                ...
                <!--- calculate and update week epf correctly --->
<!--- 				<cfif left(db,3) eq "gaf">	 --->	
					<cfset looptable = right(weekpay,1)>
                    <cfquery name="getpreepf" datasource="#db#">
                        SELECT sum(grosspay) as grosspay,sum(epfww) as epfww,sum(epfcc) as epfcc,
                        sum(epf_pay) as epf_pay FROM (
                        SELECT grosspay,epfww,epfcc,epf_pay,payyes,empno FROM payweek1
                    <cfloop from="2" to="#looptable#" index="j">
                        UNION ALL SELECT grosspay,epfww,epfcc,epf_pay,payyes,empno from payweek#j# 
                    </cfloop>
                        )as a
                        WHERE
                        payyes = "Y" AND empno = "#empno#"
					...
					<cfset PAYIN = val(PAYIN) + val(getpreepf.grosspay)>
<!---                 <cfelse>
    	            <cfset PAYIN = val(PAYIN)>
                </cfif> --->
		      ...
		      <!---   cpf amount not round --->
                ...
                <cfset epf_yer_nt_round = epf_yer_nt_round * val(oldpayin1) / val(PAYIN)>
				 ...
				 <!--- calculate and update week community fund correctly --->
<!---                 <cfif left(db,3) eq "gaf">	 --->	
					<cfset looptable = right(weekpay,1)-1>
                    <cfquery name="getpreepf2" datasource="#db#">
                        SELECT sum(epfww) as epfww,sum(epfcc) as epfcc,
                        sum(epf_pay) as epf_pay FROM (
                        SELECT epfww,epfcc,epf_pay,payyes,empno FROM payweek1
                        <cfloop from="2" to="#looptable#" index="j">
                            UNION ALL SELECT epfww,epfcc,epf_pay,payyes,empno from payweek#j# 
                        </cfloop>
                            )as a
                            WHERE
                            payyes = "Y" AND empno = "#empno#"
                    ...
                    <cfset EPFW = round(EPFW - val(getpreepf2.epfww))>
                    <cfset EPFY = round(EPFY - val(getpreepf2.epfcc))>
<!---                 <cfelse>
					<cfset EPFW = round(EPFW)>
                    <cfset EPFY = round(EPFY)>                
                </cfif>    --->
				 <!--- end of calculate and update week community fund correctly --->                         
...
<!--- 		        <cfif epf1hd eq "Y">
	      ...
	      <!---   <cfquery name="ps1" datasource="#db#">
			...
			<cfquery name="#weekpay#_qry" datasource="#db#">
				SELECT netpay,EPFCC, EPFWW,grosspay,tded FROM #weekpay# WHERE empno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#empno#">
			</cfquery>
			...
			<cfloop query="prj_rcd_qry">
				<cfset project_saly_pay = val(#weekpay#_qry.netpay) * val(prj_rcd_qry.dw_p)/val(cal_dw.dw_sum)>
				<cfset project_saly_pay = numberformat(project_saly_pay,'.__')>
				 
				<cfset PRJ_NET = val(#weekpay#_qry.grosspay) - #weekpay#_qry.tded>
				<cfif get_now_month.CPFWW_ADD_PRJ eq "1">
					<cfset PRJ_NET = PRJ_NET - val(#weekpay#_qry.epfww)>
				</cfif>
				...
				<cfif get_now_month.CPFCC_ADD_PRJ eq "1">
					<cfset PRJ_NET = PRJ_NET - val(#weekpay#_qry.epfcc)>
				</cfif>
				...
				<!---  <cfset project_saly_pay = val(#weekpay#_qry.netpay) * val(prj_rcd_qry.dw_p)/val(cal_dw.dw_sum)>
				 <cfset project_cost_pay = (val(#weekpay#_qry.netpay) + val(#weekpay#_qry.EPFCC)-val(#weekpay#_qry.EPFWW))* val(prj_rcd_qry.dw_p)/val(cal_dw.dw_sum)> --->

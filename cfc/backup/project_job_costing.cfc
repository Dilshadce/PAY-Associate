	...
	<cffunction name="cal_project" access="public" returntype="any">
		...
		<cfargument name="db_main" required="yes">	
			...
			<cfquery name="pay_qry" datasource="#db#">
				SELECT basicpay,netpay,EPFCC, EPFWW,grosspay,tded,DED102,taw FROM #qry_tbl_pay#
			...
			<cfif #gsetup_qry.proj_base_basicpay# eq 1> 
				<cfset project_grosspay = numberformat(val(pay_qry.basicpay),'.__') * numberformat(val(prj_rcd_qry.dw_p),'.__')/numberformat(val(cal_dw.dw_sum),'.__')+#ot1#+#ot2#+#ot3#+#ot4#+#ot5#+#ot6#>
			<cfelse>

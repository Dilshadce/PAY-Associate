...
<cfquery name="checkstatus" datasource="#dts_main#">
    SELECT mestatus, emailpayslip,monthend FROM gsetup2 WHERE comp_id = "#HcomID#"
</cfquery>
...
<body>
<cfoutput>
<div class="mainTitle" style="text-transform:uppercase"><cfif mon neq 13>MONTH END PROCESSING FOR #dateformat(date,'MMMM')#
                                                        <cfelse>Please proceed to do year end at HouseKeeping > Year End</cfif></div>
<div class="tabber">
...
<cfif datediff('h',checkstatus.monthend,now())lt 24>
    <cfset msg = "You Have Done Month End Earlier At #checkstatus.monthend# \nAre You Sure To Proceed To Month End Again?">
<cfelse>
    <cfset msg = "Are You Sure To Proceed To Month End?">
</cfif>
...
<tr>
  ...
  <td colspan="2">Email Pay Slip To All Active Employees (Beta)</td>
</tr>
</cfif>--->
<tr>
  <td>&nbsp;</td>
...
<tr>
  <td><input type="radio" name="uwd" id="uwd" value="figures"<!--- onclick="javascript:wd_validation();"/--->></td>
...
<cfif HUserCcode eq "MY" and Hmmonth eq "2" and Hmyear eq "2016">
<tr>
  <td colspan="2">&nbsp;</td>
  <td align="right">&nbsp;</td>
</tr>
...
<tr>
  <td><input type="checkbox" name="updateepf" id="updateepf" checked="checked" value="1" /></td>
  <td colspan="2">Update latest #epf# Rate (Mar 2016 - Dec 2017)</td>
</tr>
</cfif>
...
<cfif isdefined("form.status") and checkstatus.emailpayslip eq "Y">
<form id="FormEmail" name="FormEmail" action="monthEndEmailPayslip.cfm">
    <script type="text/javascript">
        if(confirm("Confirm To Email Pay Slip To All Active Employees?")){
            FormEmail.submit();
        }
    </script>
</form>

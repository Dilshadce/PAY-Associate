<cfquery name="assign_status" datasource="#dts_main#">
    UPDATE gsetup2 SET mestatus = 'Y' WHERE comp_id = "#HcomID#"
</cfquery>
...
<cfelseif currentURL eq "payasia">
<cfset serverhost = "localhost">
<cfset servername = "root">
...
<cfloop from="1" to="6" index="i">
	...
	<cfif #monthEndUpdate.MEUD_LEAVE# eq "Y">
		<cfset mon = #numberformat(company_details.mmonth,'00')#>
		<cfset yrs = company_details.myear>
	...
	<cfif #monthEndUpdate.MEUD_HOL# eq "Y">
		<cfif company_details.mmonth eq 13>
		<cfset mon = 1>
        <cfset yrs = company_details.myear + 1>
		<cfelse>
		<cfset mon = #numberformat(company_details.mmonth,'00')# >
		<cfset yrs = company_details.myear>
        </cfif>
	...
	<cfif #monthEndUpdate.meud_lveEnt# eq "Y">
		<cfif company_details.mmonth eq 13>
		<cfset mon = 1>
        <cfset yrs = company_details.myear + 1>
		<cfelse>
		...
		SELECT *,year(dcomm) as ydcomm, month(dcomm) as mdcomm, day(dcomm) as ddcomm, empno, almctbl, dcomm
		FROM pmast
		</cfquery>
		
		<cfloop query="emp_qry">
			<cfif #emp_qry.dcomm# neq "">
            <cfset dcommdate = Createdate(emp_qry.ydcomm, emp_qry.mdcomm, emp_qry.ddcomm)>
				<cfset curdate = Createdate(yrs, mon, '1')>
                <cfset curdate = Createdate(yrs, mon, daysinmonth(curdate))>
                <cfset m3serdate = datediff("m",dcommdate,curdate)/12>
			<cfset i = round(datediff("m",dcommdate,curdate)/12)>
            <cfif i lte 0>
            <cfset i = 1>
			</cfif>
			<cfif i gt 0>
                ...
                <cfquery name="almctbl_1" datasource="#dts#">
				SELECT alday#i# as alnew FROM ottable
				WHERE ot_cou = "#emp_qry.almctbl#"
				</cfquery>
			
            	<cfif m3serdate lt 1>
				<cfset enal = round(almctbl_1.alnew / 12 * round(datediff("m",dcommdate,curdate)))>			
                <cfelse>
                <cfset enal = almctbl_1.alnew>
                </cfif>
				<!--- <cfoutput>#emp_qry.empno# - #dcommdate# - #curdate# - #m3serdate# - #enal#<br />
</cfoutput> --->
				<cfquery name="alall" datasource="#dts#">
				UPDATE pmast
				SET alall = #enal#
				WHERE empno = "#emp_qry.empno#" AND paystatus = "A" and confid >= "#hpin#"
				</cfquery>
			<cfelse>
				<cfquery name="no_update" datasource="#dts#">
				UPDATE pmast
				SET	alall = "0.00"
				WHERE empno = "#emp_qry.empno#" AND paystatus = "A" and confid >= "#hpin#"
				</cfquery>
        ...
        <cfquery name="auditclear" datasource="#dts#">
        INSERT INTO audit_leaveent (updated_on,updated_by,module)
        VALUES
        (now(),'#GetAuthUser()#','Generate AL')
        </cfquery>
	</cfif>
    ...
    <cfquery name="uptdw1" datasource="#dts#">
        UPDATE paytra1 SET 
        dw = wday-al-ph-mc-cc-mt-mr-cl-hl-ex-pt-ad-ecl-oil-opl-ls-npl-ab-onpl-ns-rs
    </cfquery>
...
<cfquery name="update_pay_tm_NONa" datasource="#dts#">
	...
	<cfquery name="lastupdate" datasource="#dts_main#">
		UPDATE gsetup SET monthend = now() WHERE comp_id = "#HcomID#"
	</cfquery>
    
	<cfquery name="assign_status2" datasource="#dts_main#">
		UPDATE gsetup2 SET mestatus = 'N', pilotstage = 0, emailpayslip = <cfif isdefined("form.emailpayslip")>"Y"<cfelse>"N"</cfif> 

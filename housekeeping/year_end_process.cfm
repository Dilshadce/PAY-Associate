...
<cfquery name="assign_status" datasource="#dts_main#">
    UPDATE gsetup2 SET yestatus = 'Y', yearend = now() WHERE comp_id = "#HcomID#"
</cfquery>
...
<cfquery name="clearleave" datasource="#dts#">
	...
	<cfloop query="pmastqry_qry_2">
        <!---enhance for more tables [2016/01/11, by Max Tan]--->
        <cfset backuplist = "pmast,pay_12m,pay1_12m_fig,pay2_12m_fig,pay_ytd,comm_12m,bonu_12m,extr_12m,emp_users,pleave,timesheettable,claimlist">
        
        <cfloop list="#backuplist#" index="i">
            <cfquery name="backup_#i#" datasource="#dts#" >
                INSERT INTO deleted_#i#
                SELECT * FROM #i#
        ...
        <!---enhance for more tables [2016/01/11, by Max Tan]--->
        <cfset dellist = "ap8a,ap8b,tp2,tp3,pcb_12m,pcb2table,cp22a,eatable,ectable,wdtable,timesheettable,claimlist,leave_apl">	
		
        <cfloop list="#dellist#" index="i">
            <cfquery name="delete_#i#" datasource="#dts#" >
                DELETE FROM #i#
                WHERE empno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#pmastqry_qry_2.empno#">
            </cfquery>
        </cfloop>   
	...
	<cfquery name="insert_hol" datasource="#dts#">  	
        INSERT INTO holtable (hol_date,hol_desp) VALUES  
        (<cfqueryparam cfsqltype="cf_sql_varchar" value="#yrs#-01-01">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="NEW YEAR'S DAY">),
        (<cfqueryparam cfsqltype="cf_sql_varchar" value="#yrs#-05-01">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="LABOUR DAY">),
    <cfif HuserCcode eq 'SG'>
        (<cfqueryparam cfsqltype="cf_sql_varchar" value="#yrs#-08-09">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="NATIONAL DAY">),
    <cfelseif HuserCcode eq 'MY'>            
        (<cfqueryparam cfsqltype="cf_sql_varchar" value="#yrs#-08-31">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="NATIONAL DAY">),
    <cfelseif HuserCcode eq 'ID'>
        (<cfqueryparam cfsqltype="cf_sql_varchar" value="#yrs#-08-17">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="INDEPENDENCE DAY">),
    </cfif>
        (<cfqueryparam cfsqltype="cf_sql_varchar" value="#yrs#-12-25">,
        <cfqueryparam cfsqltype="cf_sql_varchar" value="CHRISTMAS DAY">)
    </cfquery>
...
<!-- 	adding the system year  -->
   ...
   <cfquery name="year_end_pleave" datasource="#dts#">
    ...
    <cfquery name="getcountry" datasource="#dts_main#">
SELECT ccode,myear FROM gsetup WHERE comp_id = "#HcomID#"
</cfquery>
	
    <cfif getcountry.ccode eq "SG" and getcountry.myear eq "2016">
    ...
    <cfquery name="updatededuction" datasource="#dts#">
    update dedtable as a, empty_p.dedtable as b SET a.ded_for = b.ded_for WHERE a.ded_cou = b.ded_cou and a.ded_desp in ("SINDA","CDAC","EURASIAN")
    </cfquery>
...
SHOW COLUMNS FROM payroll_main.gsetup where field not in ("myear","mmonth","dailyleave")
</cfquery>
...
<cfquery name="getcolumns2" datasource="#dts_main#">
SHOW COLUMNS FROM payroll_main.gsetup2 
</cfquery>

<cfquery name="deleteoldcolumn2" datasource="#dts_main#">
DELETE FROM gsetup2 WHERE comp_id = "#companyidnew#"
</cfquery>

<cfquery name="insertfrom2" datasource="#dts_main#">
INSERT INTO gsetup2
(
<cfloop query="getcolumns2">
<cfif getcolumns2.field neq "entry">
#getcolumns2.field#<cfif getcolumns2.recordcount neq getcolumns2.currentrow>,</cfif>
</cfif>
</cfloop>
)
SELECT
"#companyidnew#",
<cfloop query="getcolumns2">
<cfif getcolumns2.field neq "comp_id" and getcolumns2.field neq "entry">
#getcolumns2.field#<cfif getcolumns2.recordcount neq getcolumns2.currentrow>,</cfif>
</cfif>
</cfloop>
FROM gsetup2 WHERE comp_id = "#hcomid#" limit 1
</cfquery>

<cftry>
<cfquery name="updatecompanyid" datasource="#dts_main#">
UPDATE #companyidnew#_p.emp_users SET companyid = "#companyidnew#"
</cfquery>
<cfcatch type="any">
</cfcatch>
</cftry>
...
<br>
For employee portal login, please kindly use above company id with existing username and password will do.<br>
<br>
Best Regards,<br />
Netiquette<br />
</cfmail>
...
<cfelse>
<cfset status_msg = "Year End Completed But Create Previous Year Database failed! Please contact System Administrator!">
<h1>Year End Completed But Create Previous Year Database failed! Please contact System Administrator!</h1>
<cfabort>
</cfif>

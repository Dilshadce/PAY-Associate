	...
	truncate pay_12m ;
		...
		<cfquery name="sum_all" datasource="#dts#">
			...
			(SELECT EMPNO, TMONTH, sum(BRATE) as brate, sum(oob) as OOB, sum(wday) as WDAY, sum(DW) as dw, sum(PH) as ph, sum(AL) as al, ALHR,
				  ...
				  sum(rate6)/2 as RATE6, "#HUserName#" as update_by, "#dateformat(now(),'yyyymmdd')#" as update_on,  sum(ns) as NS, sum(ADDITIONALWAGES) as ADDITIONALWAGES,trim(GROUP_CONCAT(CHEQUE_NO SEPARATOR ' ')) as CHEQUE_NO,sum(ecl) as ECL,sum(rs) as RS,sum(oil) as OIL, sum(toff) as toff 
...
<cfquery name="update_pay_tm_NONa" datasource="#dts#">
	...
	<cfquery name="assign_status2" datasource="#dts_main#">
		UPDATE gsetup2 SET mestatus = 'N' WHERE comp_id = "#HcomID#"
	</cfquery>
</cfoutput>

<!---MT <cfscript>
	religion = event.getArg('religionData');	
	xe.submit = event.getArg('xe.submit');	
	mode = event.getArg("submitLabel");	
</cfscript>--->

<cfset currentFile="religiontableform.cfm">
<!--- <cfset currentFile="http://localhost/housekeeping/maintenance/ReligionTableForm.cfm"> --->
...
<cfquery name="getrel_qry" datasource="#dts#">
...
<cfquery name="relcount_qry" datasource="#dts#">
 	SELECT FOUND_ROWS() as records;
</cfquery>

<cfset total_pages=ceiling(relcount_qry.records/records_per_page)>
...
function confirmDelete(relcode,type) {
	var answer = confirm("Confirm Delete?")
	if (answer){
		window.location = "/housekeeping/maintenance/religionTableForm_process.cfm?type="+type+ "&relcode="+relcode;
	}
	else{
...
<body>
<!--- MT <body onLoad="document.rform.relcode.focus()"--->
<!---div class="mainTitle"><cfoutput>#event.getArg('submitLabel')#</cfoutput></div>--->
<cfinclude template="religionTableMain.cfm">
		...
		<div class="ttype3">
			...
			<span class="ttype">Search By :
				<select name="sType">
			      <option value="relcode">ID</option>
			      <option value="reldesp">Religion Description</option>	
			    </select>
				Search for : 
			...
			<!--- <div align="right"><a class="subMenu" href="custData.cfm?type=Create&en=0">Create New Customer</a></div>--->
		</div> 
		...
		<cfform name="form1" action="#currentFile#?list=" method="post" onsubmit="return checkValue();">
			...
			<table width="" class="data">
				...
				<cfif getrel_qry.recordcount>
					<cfoutput query="getrel_qry">
					<tr>
						<td width="30px">
							#getrel_qry.currentrow#
						</td>
						<td>
							#getrel_qry.relcode#
						</td>
						<td>
	           			...
	           			<td width="">
		           			<a href="religionTableForm_process.cfm?type=edit&RELCODE=#getrel_qry.RELCODE#"><img height="18px" width="18px" src="/images/edit.ICO" alt="Edit" border="0">Edit</a> || 
							<a href="##" onclick="confirmDelete('#getrel_qry.RELCODE#','delete')"><img height="18px" width="18px" src="/images/delete.ICO" alt="Delete" border="0">Delete</a>
						</td>
					</tr>
					</cfoutput>
				<cfelse>
		...
		<cfoutput>
			...
			<div class="" align="center">
				...
				<cfif begin_page*records_per_page LT relcount_qry.records>

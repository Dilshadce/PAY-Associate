...
<cfif isdefined('form.sub_btn')>
    ...
    SELECT * FROM (SELECT sum(ot)as totalot2,sum(ot)as totalot6,empno FROM #replace(dts,'_p','_i')#.emptimesheet 
    where month='#mon#' and year='#yrs#' GROUP BY empno) as a where a.totalot2 > 0
    </cfquery>
    
    <cfquery name="getsun" datasource="#dts#">
	SELECT count(day)/2 as totalot5, e.* FROM #cmpid#.emptimesheet e WHERE
    day not in (#pubdate#) and offday = "off" AND (timefrom != 0 or timeto != 0) and month= #mon# AND year= #yrs# GROUP BY empno
    </cfquery>
        
    <cfquery name="getpub" datasource="#dts#">
	SELECT count(day)/2 as totalot6, e.* FROM #cmpid#.emptimesheet e WHERE
	...
	SELECT count(day)/2 as total, e.* FROM #cmpid#.emptimesheet e WHERE
	...
	SELECT empno FROM #cmpid#.emptimesheet e WHERE month= #mon# AND year= #yrs# GROUP BY empno
    </cfquery>
...
<cfabort>   
    ...
    <cfquery name="refreshdata" datasource="#dts#">
 	UPDATE paytran set hr2=null, hr5=null, hr6=null,al=null,cl=null,hl=null,mc=null,npl=null,payyes='N',ph=0 
    WHERE empno='#refresh.empno#'
    </cfquery>
    <cfquery name="refreshdata" datasource="#dts#">
 	UPDATE paytra1 set hr2=null, hr5=null, hr6=null,al=null,cl=null,hl=null,mc=null,npl=null,payyes='N',ph=0
    WHERE empno='#refresh.empno#'
    </cfquery>
    <cfquery name="updatepub" datasource="#dts#">
    UPDATE paytra1 SET ph='#getpubdate.recordcount#', payyes = "Y" WHERE empno=<cfqueryparam cfsqltype="cf_sql_varchar" value="#refresh.empno#">
    </cfquery>

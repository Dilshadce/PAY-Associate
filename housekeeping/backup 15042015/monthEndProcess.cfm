	...
	truncate pay_12m ;
		...
		<cfquery name="sum_all" datasource="#dts#">
			...
			(SELECT EMPNO, TMONTH, sum(BRATE) as brate, sum(oob) as OOB, sum(wday) as WDAY, sum(DW) as dw, sum(PH) as ph, sum(AL) as al, ALHR,
				  ...
				  sum(rate6)/2 as RATE6, "#HUserName#" as update_by, "#dateformat(now(),'yyyymmdd')#" as update_on,  sum(ns) as NS, sum(ADDITIONALWAGES) as ADDITIONALWAGES,trim(GROUP_CONCAT(CHEQUE_NO SEPARATOR ' ')) as CHEQUE_NO,sum(ecl) as ECL,sum(rs) as RS,sum(oil) as OIL
			...
			from (select EMPNO, TMONTH, BRATE, OOB, WDAY, DW, PH, AL, ALHR, MC, MT,
				  ...
				  AR_ACC3, AR_ACC4, AR_ACC5, PAYYES, RATE1, RATE2, RATE3, RATE4, RATE5, RATE6, update_by,update_on,NS,ADDITIONALWAGES,CHEQUE_NO,ECL,OIL,RS  FROM PAY1_12M_FIG AS A
			UNION ALL SELECT EMPNO, TMONTH, BRATE, OOB, WDAY, DW, PH, AL, ALHR, MC, MT,
				  ...
				  SOCSOWW, SOCSOCC, SODSOWW, SODSOCC, SOESOWW, SOESOCC, UNIONWW, UNIONCC, CCSTAT1, PENCEN, BINK ,MFUND, DFUND,
				  EPF_PAY, EPF_PAY_A, EPF_PAY_B, EPF_PAY_C, TAWCPF, TAWCPFWW, TAWCPFCC, HRD_PAY, ACTIV_MTH, AR_ACC1, AR_ACC2,
				  AR_ACC3, AR_ACC4, AR_ACC5, PAYYES, RATE1, RATE2, RATE3, RATE4, RATE5, RATE6, update_by, update_on, NS, ADDITIONALWAGES,CHEQUE_NO,ECL,OIL,RS
...
<cfquery name="proj_rcd_qry_1" datasource="#dts#">
    ...
    netpay, userid, jobcosting, tmonth, UPDATE_BY, UPDATE_ON)
    ...
    "#dateformat(now(),'yyyymmdd')#" AS UPDATE_ON from proj_rcd
</cfquery>
...
<cfquery name="proj_pay_qry_2" datasource="#dts#">
    ...
    Select empno, project_code, date_p, dw_p, mc_p,al_p,ph_p, npl_p, ot1_p, ot2_p, ot3_p, ot4_p, ot5_p, ot6_p, 
    netpay, userid, jobcosting, "#company_details.mmonth#" as tmonth, "#HUserName#" AS UPDATE_BY, 
    "#dateformat(now(),'yyyymmdd')#" AS UPDATE_ON
    from proj_rcd_1
</cfquery>
...
UPDATE paytran SET #num_leave.LVE_TYPE# = #sum_leave.sumlve# WHERE empno = "#num_leave.empno#"
	...
	<cfif trans_type eq "RT" and isdefined("form.uwd") eq false>
...
<cfset status_msg="Fail To Month End. Error Message : #cfcatch.Detail#">
</cfcatch>
</cftry>  --->

...
<body>
	...
	<div class="tabber">
		...
		<div class="tabbertab">
            ...
            <tr>
				<td colspan="2">Auto Assign NPL From Resigned Date</td>
				<td colspan="2"><input type="checkbox" name="autonpl" id="autonpl" <cfif gs_qry.autonpl eq "Y">checked="checked"</cfif> /></td>
			</tr>
			<tr>
		...
		<div class="tabbertab">
         ...
         <tr>
            ...
            <tr>
            <td colspan="2">Reset ePortal password to </td>
            <td colspan="3"><select name="eportalpass" id="eportalpass">
            				<option value="nricn" <cfif gs_qry2.eportalpass eq "nricn">Selected</cfif>>I/C No. (New)</option>
            				<option value="random" <cfif gs_qry2.eportalpass eq "random">Selected</cfif>>Random Code (Require Email)</option>
                            </select>
            </td>
            </tr>
			<tr>
            ...
            <tr>
                ...
                <td />
                <td />
				<td><input type="checkbox" name="wpemail" id="wpemail" 
                onclick="if(document.getElementById('wpdays').value==0){wpdays.value=5}else{wpdays.value=0}" 
                <cfif gs_qry.wpemail eq "Y">checked</cfif>></td>
                <td>Remind WP expiry in <cfinput type="text" name="wpdays" id="wpdays" size ="1" maxlength = "3" validate="integer" 
                message="Remind WP expiry in numerics 1 - 999 days only" value="#gs_qry.wpexpdays#" /> days</td>  
            </tr>
            <tr>
           ...
           <tr>
                <td><input type="checkbox" name="leaveattach" id="leaveattach" <cfif gs_qry2.leaveattach eq "Y">
                checked</cfif>>Compulsory Leave Attachment For  
                    <input type="text" name="leaveattachlist" id="leaveattachlist" value="#gs_qry2.leaveattachlist#" />
                </td>                  
                <td />
                <td />
				<td><input type="checkbox" name="cancelleave" id="cancelleave" <cfif gs_qry2.cancelleave eq "Y">checked</cfif>></td>
                <td>ePortal Cancel Approved Leave</td>                  
           </tr>  
           <tr>
				<td />                  
                <td />
                <td />
				<td><input type="checkbox" name="leavedays" id="leavedays" 
                onclick="if(document.getElementById('daysinadvance').value==0){daysinadvance.value=3}else{daysinadvance.value=0}" 
                <cfif gs_qry2.leavedays eq "Y">checked</cfif>></td>
                <td>No. of Days in Advance to Apply AL/NPL: <cfinput type="text" name="daysinadvance" id="daysinadvance" size ="1" 
                maxlength = "3" validate="integer" message="Days in numerics only" value="#gs_qry2.daysinadvance#" /></td>                  
           </tr>  
        ...
        <div class="tabbertab">
            <h3>Control 2</h3>
            <table width="70%">
            <tr><th colspan="4">Auto Pilot Control</th></tr>
            <tr>
				<td><input type="checkbox" name="autopilot" id="autopilot" 
                onclick="if(document.getElementById('pilotday').value==0){pilotday.value=1}else{pilotday.value=0}" 
                <cfif gs_qry2.autopilot eq "Y">checked</cfif>>
                    Activate auto pilot and process payroll at day <select id="pilotday" name="pilotday">
                                                                    <cfloop from="1" to="31" index="i">
                                                                        <option value="#i#" <cfif gs_qry2.pilotday eq i>selected </cfif>>#i#</option>
                                                                    </cfloop>
                                                                </select>
                </td>
                <td />
                <td>Current Auto Pilot Status - <strong>#gs_qry2.pilotstage#</strong></td>
                <td />
            </tr>
            <tr>
				<td><input type="checkbox" name="informstatus" id="informstatus" 
                onclick="if(document.getElementById('informday').value==0){informday.value=2}else{informday.value=0}" 
                <cfif gs_qry2.informstatus eq "Y">checked</cfif>>
                    Email payroll reminder <select id="informday" name="informday">
                                                <cfloop from="1" to="9" index="i">
                                                    <option value="#i#" <cfif gs_qry2.informday eq i>selected </cfif>>#i#</option>
                                                </cfloop>
                                            </select> day(s) before auto pilot process
                <td />
                <td><cfif gs_qry2.pilotstage eq 0><strong></cfif>0 - Collecting Information<cfif gs_qry2.pilotstage eq 0></strong></cfif></td>
                <td />
            </tr>
            <tr>
                <td><input type="checkbox" name="emailpayslip" id="emailpayslip" <cfif gs_qry2.emailpayslip eq 'Y'>checked</cfif>>
                    Email pay slips after month end (Customised pay slip or pay slip 3 only)
                </td>
                <td />
                <td><cfif gs_qry2.pilotstage eq 1><strong></cfif>1 - Email Payroll Reminder (Optional)<cfif gs_qry2.pilotstage eq 1></strong></cfif></td>
                <td />
            </tr>
            <tr>
                <td />
                <td />
                <td><cfif gs_qry2.pilotstage eq 2><strong></cfif>2 - Processing Payroll<cfif gs_qry2.pilotstage eq 2></strong></cfif></td>
                <td />
            </tr>
            <tr>
                <td />
                <td />
                <td><cfif gs_qry2.pilotstage eq 3><strong></cfif>3 - Email Summary Report<cfif gs_qry2.pilotstage eq 3></strong></cfif></td>
                <td />
            </tr>    
            </table>    
        </div>
		...
		<div class="tabbertab">
			...
			<tr>
                <td colspan="2">Default Payslip for ePortal</td>
                <td colspan="2">
                    <select name="pstype" id="pstype">
                        <cfloop from="1" to="3" index="i">
                            <option value="#i#" <cfif gs_qry2.pstype eq i>selected</cfif>>#i#</option>
                        </cfloop>
                    </select>
                </td>
            </tr>

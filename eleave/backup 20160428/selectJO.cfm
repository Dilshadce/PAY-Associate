...
<cfset endno = mid(trim(getplacementlist.timesheet),4,2)>
        ...
        <cfif isdefined("url.type")>
            ...
            <cfelseif url.type eq "claim">
                <cfset path = "/employee/createlist.cfm?type=mcreate">
            <cfelseif url.type eq "incident">
                <cfset path = "/eleave/incident/incidentmain.cfm">
            </cfif>
        </cfif>
        <td><form action="#path#" method="post"><input type="hidden" name="pno" id="pno" value="#evaluate('placementno#a#')#"><input type="hidden" name="tsdates" id="tsdates" value="#dateformat(evaluate('startdate#a#'),'yyyy-mm-dd')#"><input type="hidden" name="tsdatee" id="tsdatee" value="#dateformat(evaluate('enddate#a#'),'yyyy-mm-dd')#">
         <cfset currentdate = createdate(val(company_details.myear),val(company_details.mmonth),1)>
         <cfif a eq 10>
		 <cfset currentdate = dateadd('m',1,currentdate)>
		 </cfif>
         <input type="hidden" name="nexmonth" id="nexmonth" value="#dateformat(currentdate,'m')#">
        <input type="submit" name="sub_btn" value="Pick"></form></td>
        </tr>
		</cfif>
        ...
        <tr>
        <td colspan="6"><h1>Online timesheet is not applicable at this moment.  Please contact xxx at 99999999 during office hour (9am to 6pm) if you have further queries.</h1></td>
        </tr>
		</cfif>
        </table>

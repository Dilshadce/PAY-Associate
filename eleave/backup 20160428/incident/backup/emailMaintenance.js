...
$(document).ready(function(e) {
	var resultTable = $('#emailTable').dataTable({
		'scrollX':true,
			...
			{'aTargets':[5],'sTitle':("action").toUpperCase(),'mData':'id','bSortable':false,'bVisible':true,'sWidth':'10%',
			                         'mRender':function(data,type,row){
										 if(row.type != "Birthday"){
											 if(row.status != "SENT"){
												 if(row.schedule != ""){
								   	 	 			return '<span class="glyphicon glyphicon-pencil btn btn-link" onclick="window.open(\'emailDetail.cfm?action=edit&id='+escape(encodeURIComponent(data))+'\',\'_self\');"></span>'+
											'<span class="glyphicon glyphicon-remove btn btn-link" onclick="if(confirm(\'Are you sure you wish to delete this?\')){window.open(\'emailMaintenanceProcess.cfm?action=delete&id='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>';
												 }
												 else{
													 return '<span class="glyphicon glyphicon-pencil btn btn-link" onclick="window.open(\'emailDetail.cfm?action=edit&id='+escape(encodeURIComponent(data))+'\',\'_self\');"></span>'+
											'<span class="glyphicon glyphicon-remove btn btn-link" onclick="if(confirm(\'Are you sure you wish to delete this?\')){window.open(\'emailMaintenanceProcess.cfm?action=delete&id='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>'+
											'<span class="glyphicon glyphicon-send btn btn-link" onclick="if(confirm(\'Are you sure want to send this?\')){window.open(\'selfSend.cfm?id='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>';
												...
												return '<span class="glyphicon glyphicon-remove btn btn-link" onclick="if(confirm(\'Are you sure you wish to delete this?\')){window.open(\'emailMaintenanceProcess.cfm?action=delete&id='+escape(encodeURIComponent(data))+'\',\'_self\');}"></span>';
											}
										 }
										 else{
											return 	'<span class="glyphicon glyphicon-pencil btn btn-link" onclick="window.open(\'emailDetail.cfm?action=edit&id='+escape(encodeURIComponent(data))+'\',\'_self\');"></span>'
		...
		'sAjaxSource':'emailMaintenance.cfc',
		'sServerMethod':'POST',
        'sScrollX':'100%'
	})
	.fadeIn();

...
function validate_form(thisform)
	...
	<!--- 	var days = document.eForm.days.value;
		...
		var timefr_t = document.getElementById('timeFr_t').value;
		var timeto_t = document.getElementById('timeTo_t').value;
...
<cfset bal_AL = val(emp_data.alall)-(val(leave_data.AL)+val(tLVE_DAY_b)) >
<cfelse>
<cfset bal_AL = val(emp_data.alall)-(val(tLVE_DAY_a)+val(tLVE_DAY_b))>
</cfif>
...
<div class="tabber">
        ...
        <td width="5px">:</td>
        <td><input type="text" name="empno" id="empno" value="#emp_data.empno#" readonly="readonly" /></td>
		<td></td>
        <cfif aLeave_qry.earnleave eq 1>
        <th>Annual Leave Available</th>
        <td>:</td>
        <td><input type="text" name="bal_avl" id="bal_avl" value="#bal_avl#" readonly="readonly" /></td>
        <cfelse>
        <td></td>
        </cfif>
        ...
        <tr>
          <th>Name</th>
          <td>:</td>
          <td><input type="text" name="name" value="#emp_data.name#" id="name" readonly="readonly" /></td>
		<td></td>
		<th>Annual Leave Balance</th>
		<td>:</td>
		<td><input type="text" name="bal_al" id="bal_al" value="#bal_al#" readonly="readonly" /></td>
        </tr>
        <tr>
          <th>Leave Type</th>
          <td>:</td>
          <td><select id="leaveType" name="leaveType" 
          onchange="if(this.value == 'NCL' || this.value == 'TOff'){show_row('time');} else {show_row('days');}
                    <cfif comp_details.leaveattach eq "Y" and comp_details.leaveattachlist neq "">
                    <cfset count = 0>
                    <cfloop list='#comp_details.leaveattachlist#' index='i'>
                        <cfif count eq 0>
                        if(this.value=='#trim(ucase(i))#'
                        <cfelseif count gte 1 and count neq listlen(comp_details.leaveattachlist)>
                        ||this.value=='#trim(ucase(i))#'
                        </cfif>
                        <cfset count += 1>
                        <cfif count eq listlen(comp_details.leaveattachlist)>
                        ){show_attach('yes');}else{show_attach('no');}
                        </cfif>
                    </cfloop>
                    </cfif>
                        " Onclick="changeTozero()">
					<option id="AL" value="AL" selected>Annual Leave</option>
					...
					<option id="PH" value="PH">Public Holiday Leave</option>
					<option id="AD" value="AD">Advance Leave</option>
                    <option id="ECL" value="ECL">Extended Childcare Leave</option>
                    <option id="OIL" value="OIL">Off in Lieu</option>
                    <option id="RS" value="RS">Reservist</option>
					...
					<cfif comp_details.ncltoal eq 'N'>
                    <option id="TOff" value="TOff">Time Off</option>
					<cfelse>
		...
		<td>:</td>
		<td><input type="text" name="bal_mc" id="bal_mc" value="#bal_MC#" readonly="readonly" /></td>
        </tr>
		<td></td>
        </tr>
        <tr>
         ...
         <td><input type="text" size="15" name="dateFrom" id="dateFrom" value=""  readonly="readonly"/>
			<img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(document.getElementById('dateFrom'));
                    ...
                    <cfloop list='#comp_details.leaveattachlist#' index='i'>
                        <cfif count eq 0>
                        if(document.getElementById('leaveType').value=='#trim(ucase(i))#'
                        <cfelseif count gte 1 and count neq listlen(comp_details.leaveattachlist)>
                        ||document.getElementById('leaveType').value=='#trim(ucase(i))#'
                        </cfif>
                        <cfset count += 1>
                        <cfif count eq listlen(comp_details.leaveattachlist)>
                        ){show_attach('yes');}else{show_attach('no');}
		...
		<td><input type="text" id="ccbal" name="ccbal" value="#bal_CC#"  readonly="readonly"/></td>
          ...
          <th>Date To</th>
          <td>:</td>
          <td><input type="text" size="15" name="dateTo" id="dateTo" value="" readonly="readonly" />
        ...
        <td>:</td>
		<td>
        <input type="text" name="leave_option" id="leave_option" value="Days" readonly="readonly" />
     ...
     <tr id="dayoption">
          ...
          <div id="ajaxdate">
        	<input type="text" id="days_d" name="days" value="1" <cfif aLeave_qry.c_leavebalance neq '1'>onkeyup="validhalf(this.value);controlbalance();"<cfelse>onkeyup="validhalf(this.value);"</cfif> /><!--- <cfif DSNAME eq 'uniq_p' or DSNAME eq 'maven_p' or DSNAME eq 'kjcpl_p' or DSNAME eq 'viva_p' or DSNAME eq 'mlpl_p'>readonly</cfif> --->
            </div>
	    <cfif DSNAME eq 'netsource_p'>
	    <input type="checkbox" name="halfdayonly" id="halfdayonly" onclick="if(this.checked == true){document.getElementById('days_d').value=0.5;validhalf(document.getElementById('days_d').value);document.getElementById('timeFr_d').value='09:00';document.getElementById('timeTo_d').value='13:00';}else{document.getElementById('days_d').value=1;validhalf(document.getElementById('days_d').value);document.getElementById('timeFr_d').value='';document.getElementById('timeTo_d').value='';}" />(Half Day)
	    <cfelse>
            <input type="checkbox" name="halfdayonly" id="halfdayonly" onclick="if(this.checked == true){document.getElementById('days_d').value=0.5;validhalf(document.getElementById('days_d').value);document.getElementById('timeFr_d').value='08:00';document.getElementById('timeTo_d').value='11:59';}else{document.getElementById('days_d').value=1;validhalf(document.getElementById('days_d').value);document.getElementById('timeFr_d').value='';document.getElementById('timeTo_d').value='';}" />(Half Day)
	    </cfif>
	    ...
	    <cfelse>
            AM<input type="radio" name="halfday" id="halfday" value="am" checked="checked" onclick="if(this.checked){document.getElementById('timeFr_d').value='08:00';document.getElementById('timeTo_d').value='11:59';}" />
            &nbsp;&nbsp;&nbsp;PM<input type="radio" name="halfday" id="halfday" value="pm" onclick="if(this.checked){document.getElementById('timeFr_d').value='12:00';document.getElementById('timeTo_d').value='17:00';}" />
	    </cfif>
            <br />
			Time From : <input name="timeFr" type="text" id="timeFr_d" value=""/><br/>
            Time To : <input name="timeTo" type="text" id="timeTo_d" value=""/></div>
          <!---  <span id="sprytextfield1"> <span class="textfieldRequiredMsg">A value is required.</span>
...
<a href="uploadImages/index.cfm" target="_blank" >Upload Images </a>
 	...
 	<tr id="attach" style="display:none">
        <th >Attachment</th>
        <td >:</td>
        <td colspan="4"><input type="file" name="attachfield" id="attachfield" /></td>
<!---		<td></td>
		<td></td>
		<td></td>
		<td></td>--->

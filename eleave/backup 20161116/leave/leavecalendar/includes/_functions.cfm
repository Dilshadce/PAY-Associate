...
<cffunction
	...
	--->
	<cfquery name="LOCAL.RawEvent" datasource="#dsname#">
		SELECT
			e.id,
			e.name,
	...
	--->
	<cfquery name="LOCAL.EventException" datasource="#dsname#">
		SELECT
			e.date,
			e.event_id
		FROM
		...
		<cfif LOCAL.RawEvent.repeat_type>
			...
			<cfloop condition="(LOCAL.Day LTE LOCAL.To)">
				...
				<cfif (
					...
					<cfset LOCAL.Event[ "agent" ][ LOCAL.Event.RecordCount ] = LOCAL.RawEvent.agent />
					<!--- 

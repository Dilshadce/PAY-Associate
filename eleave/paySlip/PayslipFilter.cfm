...
<!---dsname is _p--->
    ...
    <!---To modify datasource chose based on year, [20180110, Alvin]--->
    <cfif "#form.yearchose#" NEQ "#company_details.myear#">
        <cfset dts = "#Replace(dsname, '_p', '#Right(form.yearchose, 2)#_i')#">
        <cfset dsname = "#Replace(dsname, '_', '#Right(form.yearchose, 2)#_')#">
    <cfelse>
        <cfset dts = "#Replace(dsname, '_p', '_i')#">
    </cfif>
    <!---Modify datasource year--->
    ...
    <cfquery name="checkPay" datasource="#dts#">
        ...
        date_add(c.giropaydate, INTERVAL 5 DAY) AS avail_date
        FROM assignmentslip a
        LEFT JOIN argiro b ON a.batches = b.batchno
        ...
        AND a.payrollperiod = "#form.month#"
        AND b.appstatus = "Approved"
        AND b.generated_on != "0000-00-00 00:00:00"
        ORDER BY avail_date DESC
    </cfquery>
    
    <cfif #checkPay.recordcount# NEQ 0 AND ("#checkPay.girorefno#" NEQ "" OR ("#checkPay.icgenerated#" LTE now() AND "#checkPay.icgenerated#" NEQ "") OR ("#checkPay.argenerated#" LTE now() AND "#checkPay.argenerated#" NEQ ""))
        AND #DateFormat(now(), 'yyyy-mm-dd')# GTE #DateFormat(checkPay.avail_date, 'yyyy-mm-dd')#> 

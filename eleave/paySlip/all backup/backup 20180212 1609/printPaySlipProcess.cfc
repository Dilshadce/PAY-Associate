		...
		for(key in allowances){
			if(allowances[key] > 0){
                if(right(key,1) == 9 && len(key) == 4 && getAssignment.custno == '300033162'){
                    /*ArrayAppend(earnings,getAllowance(right(key,1),dts));*/
                }else{
...
<cffunction name="sendMail" access="remote" > <!---remote, returnFormat="json"--->
    ...
    <cftry>
    	<cfmail from="donotreply@manpower.com.my" to="#trim(empPmast.email_address)#" failto="mymppayroll@manpower.com.my" bcc="alvinh.mpg@gmail.com" subject="E-Payslip #MonthAsString(month)# #year#" type="HTML">
			<p>Dear Associate, </p>
			<br>

<cfquery datasource="#dsname#" name="getEmp">
	SELECT empno,
	(SELECT nricn FROM pmast where empno = emp_users.empno) as nric FROM emp_users where userName = '#getAuthUser()#'
</cfquery>
...
<cfquery name="getEntity" datasource="#dts#">
	...
	<cfscript>
		...
		for(key in allowances){
			if(allowances[key] > 0){
				...
				ArrayAppend(deductions_amt,(allowances[key] * -1));
				}
		}
		...
		// Y-T-D amount
		 ...
		 nettPay = getAssignment.sum_selftotal - getAssignment.DED115;
	...
	<cfreport template="payslip.cfr" permissions="allowprinting" filename="#getEmp.empno#_payslip.pdf"  format="PDF" userpassword="#getEmp.nric#"
	overwrite="yes"
	encryption="128-bit"
	>
...
<cfcontent type="application/octet-stream" file="C:/inetpub/wwwroot/PAY-Associate/eleave/paySlip/#getEmp.empno#_payslip.pdf" deletefile="Yes">

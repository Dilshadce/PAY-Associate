...
function getLeaveBalance(leaveType)
{
	$.ajax({
	  ...
	  , data: {
        ...
        , leavestart: document.getElementById('dateFrom').value
        , leaveend: document.getElementById('dateTo').value
	  }
	  , success: function (data){
		$("#leavebalance").val(data["balance"]);
		$("#totalearnedleaves").val(data["earnedleaves"]);
		$("#totaltakenleaves").val(data["totaltaken"]);
		$("#currentleavetype").val(data["earningleave"]);
		$("#totalentitlement").val(data["leaveentitlement"]);
          
        $('#leaveapplytype').val("Entitled");
        //$('#leaveapplytype').removeClass("highlightleave");
...
function leaveApplyType()
{
    if($('#dateTo').val() != "" && $('#dateFrom').val() != "" && $("#currentleavetype").val() == "Y")
    {
        if(parseFloat($('#days_d').val())+parseFloat($('#totaltakenleaves').val()) > parseFloat($('#totalearnedleaves').val()))
        {
            if($('#leaveapplytype').val() != 'Advance leaves'){
                $.alert({
                    type: 'orange',
                    theme: 'light',
                    typeAnimated: true,
                    useBootstrap: false,
                    boxWidth: '40%',
                    title: 'ADVANCE LEAVE ALERT!',
                    content: 'The leave date/s applied has not been earned. Any unearned leave consumed will be regarded as Advance Leave.'
                });
            }
            $('#leaveapplytype').val("Advance leaves");
            //$('#leaveapplytype').addClass("highlightleave");

        }
        else
        {
            $('#leaveapplytype').val("Earned leave");
            //$('#leaveapplytype').removeClass("highlightleave");
        }
    }
}
...
{
    if(leaveType == 'MC' || leaveType == 'EL' || leaveType == 'HL' || leaveType == 'MR' || leaveType == 'MT' || leaveType == 'CP' || leaveType == 'PT')
    {
...
function caldays()
{
    ...
    if(document.getElementById('leaveType').value == 'HL' || document.getElementById('leaveType').value == 'MT'){
        var sat = 6;
        var sun = 0;
    }
...
<style>    
    .highlightleave {
        border: 1px solid rgba(231, 124, 34, 1);
        box-shadow: 0 0 5px rgba(231, 124, 34, 1);
...
<div class="tabber">
		...
		<td><input type="text" name="bal_al" id="bal_al" value="#bal_al#" readonly /></td>--->
          ...
          <td>:</td>
          <td><input type="text" readonly id="totalentitlement" value=""></td>
        </tr>
        <tr>
          <th>Leave Type</th>
          <td>:</td>
          <td><select id="leaveType" name="leaveType" onChange="showMCAttachment(this.value);caldays();">
        ...
        <th style="width: 130px">Leave Taken</th> 
		<td>:</td>
        <td> <input type="text" readonly id="totaltakenleaves" value="0"> </td>
        </tr>
        <tr>
          <th>Date From</th>
          <td>:</td>
         <td><input type="text" size="15" name="dateFrom" id="dateFrom" value="#DateFormat(now(), 'dd/mm/yyyy')#" readonly="readonly"/>
        ...
        <tr>
            ...
            <td>:</td>
            <td>
                <input type="text" size="15" name="dateTo" id="dateTo" value="#DateFormat(now(), 'dd/mm/yyyy')#" readonly />
            ...
            <th class="earnleavesitem">Earned Leave</th>
            <td class="earnleavesitem">:</td>
            <td class="earnleavesitem"><input type="text" readonly id="totalearnedleaves" value="0"></td>
        ...
        <th style="display:none;">Current application type</th>
        <td style="display:none;">:</td>
        <td style="display:none;">
            <input type="text" readonly id="leaveapplytype" value="Entitled">
            <input type="hidden" readonly id="currentleavetype" value="">
        </td>
        
		</tr>
     <tr id="dayoption">
          <th width="20%">Days Applied</th>
          <td width="8%">:</td>

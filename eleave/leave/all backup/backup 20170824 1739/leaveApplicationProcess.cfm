...
<cfif #form.leavebalance# lt #form.days_d# AND #form.leaveType# neq 'NPL' > <!---leave insufficient--->
	<script>
...
<cfelse>
    ...
    SELECT empno, placementno FROM placement WHERE placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.pno#">
    </cfquery>
    ...
    <cfif "#form.leaveType#" EQ 'MC' AND #HComID# EQ 'manpowertest'>
       
        <cfset uploaddir = "/upload/#dsname#/leave/#getempno.empno#/#getempno.placementno#/">
        <cfset uploaddir = expandpath("#uploaddir#")> 
        <cfif directoryexists(uploaddir) eq false>
            <cfdirectory action="create" directory="#uploaddir#" >
        </cfif>
        
        <cfif isdefined('form.attachfield')>
            <cfset sizeLimit = 10240000 />
            <cfset fileInfo = GetFileInfo(GetTempDirectory() & GetFileFromPath(form.attachfield)) >
            <cfif fileInfo.size Gt sizeLimit>
                ...
                <cfif form.attachfield neq "">
                    <cftry>
                        <cffile action="upload" destination="#uploaddir#" nameconflict="makeunique" filefield="attachfield" accept="image/*,application/pdf,application/msword,application/excel,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" >
                    <cfcatch type="any">
                        <script type="text/javascript">
    ...
    <cfif mstaff eq "N">
        ...
        <cfif isdefined('form.halfday') eq false>
            <cfif hours eq "00:00">
                <cfif isdefined('form.days_d') eq false>
        ...
        INSERT into leavelist (placementno,leavetype,days,submitted,remarks,submited_on,submit_date,startdate,enddate,
            contractenddate,startampm,endampm,status <cfif isdefined('file.serverfile')>,signdoc</cfif>) <!---leavebalance,timefr,timeto--->
        values
        ('#form.pno#','#leaveType#','#val(days_d)#','Y','#applicant_remarks#',now(),now(),'#cfc_lfdate#','#cfc_ltdate#','#cfc_condate#'
        ,'#timefr#','#timeto#','IN PROGRESS' <cfif isdefined('file.serverfile')>,<cfqueryparam cfsqltype="cf_sql_varchar" value="#file.serverfile#"></cfif>)
        </cfquery>
        ...
        <cfset templatelist1 = "&amp;empno&amp;,&amp;name&amp;,&amp;leavetype&amp;,&amp;startdatet&amp;,&amp;enddate&amp;,&amp;days&amp;,&amp;startampm&amp;,&amp;endampm&amp;,&amp;remarks&amp;,&amp;hcomid&amp;">

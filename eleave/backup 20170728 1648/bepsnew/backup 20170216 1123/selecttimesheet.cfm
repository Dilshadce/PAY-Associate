...
<link href="/stylesheet/app.css" rel="stylesheet" type="text/css" >
        ...
        <cfset itemcounter = 0>							<!---item counter--->
        <cfset timesheetChanged = FALSE>				<!---first time initialisation flag--->
        ...
        <cfloop query="checkplacementlist">
            ...
            <cfif timesheetChanged eq FALSE>																								<!---this only need to be initialise one time--->
				<cfif startno gt endno>																										<!---set the loopdate and control date - loopdate will be psat month and control date will be future 1 month--->
                    <cfset controlDate = #currentdate#>																						<!---if cycle not 01-31 it loopdate wil be 2 month past from current--->	
        	...
        	<cfloop condition="#loopDate# lte #controlDate#">
                ...
                <cfset 'startdate#itemcounter#' = createdate(year(loopDate),month(loopDate),val(startno))>									<!---create start date based on the cycle and mmonth, myear, startno(timesheet cycle)--->
                <cfset 'enddate#itemcounter#' = createdate(year(nextMonth),month(nextMonth),val(endno))>									<!---create end date based on the cycle and mmonth, myear, endno(timesheet cycle)--->

                <cfif checkplacementlist.completedate lte evaluate("enddate#itemcounter#")>
                    <cfset 'enddate#itemcounter#' = dateformat(checkplacementlist.completedate, 'yyyy-mm-dd')>								<!---if cycle exceed complete date, stops at complete date--->
                ...
                <cfif checkplacementlist.startdate gt evaluate('startdate#itemcounter#')>													<!---if startdate has not begin from the defaulted startdate, set to placement startdate--->
                    <cfset 'startdate#itemcounter#' = checkplacementlist.startdate>															<!---startdate havent commence from defaulted startdate--->
                <cfelseif (month(#checkplacementlist.startdate#) eq evaluate('month(startdate#itemcounter#)') 								<!---to check month is equal for loopdate and startdate month--->
							AND year(#checkplacementlist.startdate#) eq evaluate('year(startdate#itemcounter#)')) 							<!---to check year is equal for loopdate and startdate year--->
                    ...
                    <cfset 'enddate#itemcounter#' = createdate(year(loopDate),month(loopDate),val(endno))>									<!---enddate set to earlier startdate month,year and timesheet endno--->
                    <cfset itemcounter += 1>
                	<cfset timesheetChanged = TRUE>
        				...
        				<cfloop from="0" to="#itemcounter#" index="a">
                            ...
                            <tr>
                                ...
                                <!---modified query and changed status retrieval method, [20170125, Alvin]--->
                                    ...
                                    <cfelseif #submittedCount# eq (val(daysCount) + 1)>																				<!---submitted status eq to total days specified in timesheet means all timesheet approved--->
                                    
                                        #gettimesheetstatus.status#
										<cfif gettimesheetstatus.MGMTREMARKS neq ""> - #gettimesheetstatus.MGMTREMARKS#</cfif>
										<cfif gettimesheetstatus.MPREMARKS neq ""> - #gettimesheetstatus.MPREMARKS#</cfif>
                                        
                                    <cfelseif #validatedCount# eq (val(daysCount) + 1)>																				<!---validated status eq total days specified in timesheet means all timesheet approved--->
                                    
                                        #gettimesheetstatus.status#
										<cfif gettimesheetstatus.MGMTREMARKS neq ""> - #gettimesheetstatus.MGMTREMARKS#</cfif>
										<cfif gettimesheetstatus.MPREMARKS neq ""> - #gettimesheetstatus.MPREMARKS#</cfif>
                                        
                                    <cfelseif #rejectedCount# eq (val(daysCount) + 1)>																				<!---rejected status eq total days specified in timesheet means all timesheet approved--->

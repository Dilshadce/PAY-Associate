...
(Full Day/AM/PM)</td>
 ...
 <cfset startcount = 1>
    ...
    <cfif gettimesheet.recordcount neq 0> <!---record found in database, retrieve what is in database only regardless full month or 1-2 days--->
    <!--- <input type="hidden" name="updatefile" id="updatefile" value=""> --->
        <cfloop query="gettimesheet">
            ...
            <select name="leavetype#gettimesheet.tsrowcount#" id="leavetype#gettimesheet.tsrowcount#" onChange="if(this.value != 'PH' && this.value != '' && this.value != 'OD' && this.value != 'RD'){document.getElementById('startampm#gettimesheet.tsrowcount#').style.display='block';document.getElementById('remarks#gettimesheet.tsrowcount#').value=this.options[this.selectedIndex].id} else {document.getElementById('startampm#gettimesheet.tsrowcount#').style.display='none';document.getElementById('remarks#gettimesheet.tsrowcount#').value='';}SetLeaveTime(this.value, 'startampm#gettimesheet.tsrowcount#', 'hr#gettimesheet.tsrowcount#', 'mina#gettimesheet.tsrowcount#', 'hre#gettimesheet.tsrowcount#', 'minae#gettimesheet.tsrowcount#', 'break#gettimesheet.tsrowcount#');makehour('#gettimesheet.tsrowcount#');">
            <!---added javascript--->
            ...
            <select name="break#gettimesheet.tsrowcount#" id="break#gettimesheet.tsrowcount#" onChange="makehour('#gettimesheet.tsrowcount#')">
            <cfset timenow = createdatetime('2013','1','1','0','0','0')>
            ...
            <cfset gettimesheet.othour = trim(ReReplaceNoCase(gettimesheet.othour,"[^0-9\.]","","ALL"))>
    ...
    <cfelse><!---when no data this will be executed---> <!---no timesheet found in database, create a template for user--->
        ...
        <cfif startmonth neq dateformat(currentdate,'Mmm - yyyy')>#dateformat(currentdate,'Mmm - yy')#<cfset startmonth = dateformat(currentdate,'Mmm - yyyy')></cfif></td>
        ...
        <select name="leavetype#i#" id="leavetype#i#" onChange="if(this.value != 'PH' && this.value != '' && this.value != 'OD' && this.value != 'RD'){document.getElementById('startampm#i#').style.display='block';document.getElementById('remarks#i#').value=this.options[this.selectedIndex].id} else {document.getElementById('startampm#i#').style.display='none';document.getElementById('remarks#i#').value='';}SetLeaveTime(this.value, 'startampm#i#', 'hr#i#', 'mina#i#', 'hre#i#', 'minae#i#', 'break#i#');makehour('#i#');">
        <!---added javascript--->
		<!---   <cfif pholiday eq 1 >
        <option value="PH">PH</option>
        <cfelse> --->
        ...
        <select name="break#i#" id="break#i#" onChange="makehour('#i#');">
        ...
        <input type="text" name="totalhour#i#" id="totalhour#i#" value="<cfif pholiday neq 1 >#numberformat(evaluate('getottable.workh#DayOfWeek(currentdate)#'),'.__')#<cfelse>0.00</cfif>" size="4" readonly ></td>
...
<!---  <tr height=20 style='height:15.0pt'>
 ...
 <tr>
  <td class=xl158828 align="center" colspan="18"><input type="button" name="save" id="save" onClick="
                    ...
                    <cfelse>savebtn();this.disabled = true; 
                    </cfif>" value="Save">
        <input type="button" name="sub_btn" id="sub_btn" onClick="
                    <cfif gettimesheet.recordcount neq 0 and gettimesheet.editable eq 'N'>alert('Timesheet Has Been Submitted, No Update is Allow')
                    <cfelse>document.getElementById('form1').submit();this.disabled = true;

...
<link href="/stylesheet/app.css" rel="stylesheet" type="text/css" >
        ...
        <cfloop query="checkplacementlist">
            ...
            <cfif (timesheetChanged eq TRUE) AND (#month(checkplacementlist.startdate)# neq #month(loopDate)#)>								<!---startmonth not same with loopdate month, timesheet changed--->								
				<cfif startno gt endno>																										<!---to cater for scenario where placement ended and next placement start next month--->
                	<cfset loopDate = #dateformat(dateadd('m', 1, loopDate), 'yyyy-mm-dd')#>												<!---if startno > endno means cycle is not 01-31 thus nextmonth must always be gt loopdate--->
                    <cfset nextMonth = #dateformat(dateadd('m', 1, nextMonth), 'yyyy-mm-dd')#>
                <cfelse>
                	<cfset loopDate = #dateformat(dateadd('m', 1, loopDate), 'yyyy-mm-dd')#	>												<!---else it is 01-31 thus loopdate need to be same with next month so just +1 loopdate--->
                </cfif>													
            </cfif>																															<!---loopdate add 1 month so it will be same with next month--->

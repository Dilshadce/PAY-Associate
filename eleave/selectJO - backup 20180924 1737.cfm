...
<link href="/stylesheet/app.css" rel="stylesheet" type="text/css" >
        ...
        <!---Added filter to search for placement within now(), [20180109, Alvin]--->
        <cfquery name="checkplacementlist" datasource="#dts#">
        SELECT * FROM placement WHERE (empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#emp_data.empno#"> and jobstatus = 2
        ...
        (empno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#emp_data.empno#"> 
         and jobstatus = 2 and startdate <= "#DateFormat(now(), 'yyyy-mm-dd')#" and completedate >= "#DateFormat(now(), 'yyyy-mm-dd')#")
        </cfquery>
        <!---Added filter to search for placement within now()--->

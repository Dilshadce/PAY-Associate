...
<body>
    ...
    <cfif IsDefined('url.fromMainPage')>
        <form action="/ELeave/personal/personalAccountProcess.cfm?fromMainPage=1" method="post" onsubmit="javascript: return validatepass();">
    <cfelse>
        <form action="/ELeave/personal/personalAccountProcess.cfm" method="post" onsubmit="javascript: return validatepass();">
    </cfif>
        ...
        <table>
            <tr>
                ...
                <td width="300px"><input name="opass" id="opass" type="password" required /></td>
            </tr>
            <tr>
                <div id="passwordAjaxfield"></div>
                <td width="150px">Enter New Password</td>
                <td width="300px">
                    <!---<cfinput name="npass" id="npass" type="password" validate="regex" pattern="^(?=.*[a-zA-Z])(?=.*[0-9])" message="Must be alphanumeric!" />--->
            ...
            <tr>
                ...
                <td width="300px">
                    <!---<cfinput name="npassagain" id="npassagain" type="password" validate="regex" pattern="^(?=.*[a-zA-Z])(?=.*[0-9])" message="Must be alphanumeric!" />--->
                    <input name="npassagain" id="npassagain" type="password" required
                    title="Password must contain (UpperCase, LowerCase, Number/SpecialChar and min 8 Chars)"  
                    pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$"  
                    onchange="this.setCustomValidity(this.validity.patternMismatch ? this.title : '');"/>
                </td>
        ...
        <br />
        <br />
        &nbsp;&nbsp;&nbsp;<input type="submit" name="changepass" id="changepass" value="Save" size="6"> 
    </form>
    </cfoutput>
</body>
</html>
...
<link href="/stylesheet/app.css" rel="stylesheet" type="text/css" >
        ...
        <cfloop query="checkplacementlist">
            ...
            <cfif #previousTimesheet# neq  #checkplacementlist.timesheet#>																	<!---if previous placement timesheet cycle different than current--->
            	<cfif #left(checkplacementlist.timesheet, 2)# lt #right(checkplacementlist.timesheet, 2)#>									<!---if it is 01-31, add 1 month to loopdate to make it same as nextmonth--->
            ...
            <cfif createdate(year(loopDate),month(loopDate),val(startno)) gt #checkplacementlist.completedate#>                             <!---if loop date(exact date for timesheet not 01-31 greater than completedate, timesheet expired--->
                <cfset timesheetChanged = TRUE>
                <cfcontinue>
            </cfif>
        	...
        	<cfloop condition="#loopDate# lte #controlDate#">
                ...
                <cfif #month(loopdate)# LT #month(checkplacementlist.startdate)# AND #year(loopdate)# EQ #year(checkplacementlist.startdate)#>
                	<cfset loopDate = #dateformat(dateadd('m', 1, loopDate), 'yyyy-mm-dd')#>												<!---Check if startdate havent commence from loopdate then go to the next month , [20170727, Alvin]--->

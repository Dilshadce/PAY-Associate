...
<cfquery name="getplacementlist" datasource="#dts#">
    ...
    AND startdate <= "#dateformat(startdate,'yyyy-mm-dd')#" and completedate >= "#dateformat(enddate,'yyyy-mm-dd')#" and placementno = <cfqueryparam cfsqltype="cf_sql_varchar" value="#form.pno#">
</cfquery>
...
<!---Block timesheet time in/out/break/save/submit, [20180427, Alvin]--->
<cfoutput>
    <cfquery name="getblockid" datasource="#dsname#">
        SELECT * FROM timesheet_block 
        WHERE block_id = "#getplacementlist.custno#"
        OR block_id = "#emp_data.empno#"
        ORDER BY priority ASC
    </cfquery>

    <cfif "#getblockid.recordcount#" NEQ 0 AND "#getblockid.in_out_block#" EQ "Y">
        <cfset blockid = "#getblockid.block_id#">
        <cfset blockid1 = "#getblockid.block_id#">
    <cfelse>
        <cfset blockid = "0">
        <cfset blockid1 = "1">
    </cfif>

    <input type="hidden" id="custno" value="#blockid#">
    <input type="hidden" id="exceptioncustno" value="#blockid1#">
</cfoutput>
<!---Block timesheet time in/out/break/save/submit--->
...
<cfquery name="getottable" datasource="#dts#">
    ...
    WHERE trim(sizeid) = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Trim(getplacementdetail.ottable)#">
</cfquery>
...
<head>
    <script type="text/javascript">
        $(document).ready(function(){
            <cfif "#getblockid.recordcount#" NEQ 0>                
                <cfif "#getblockid.ot_type_block#" EQ "Y">
                    $(".inputOT_type").prop("disabled", true);    
                </cfif>
                
                <cfif "#getblockid.day_type_block#" EQ "Y">
                    $(".inputDayType").prop("disabled", true); 
                </cfif>
                
                <cfif "#getblockid.in_out_block#" EQ "Y">
                    $(".inputTime").prop("disabled", true);
                </cfif>
                    
                <cfif "#getblockid.save_submit_block#" EQ "Y" AND "#Evaluate('getblockid.month#form.nexmonth#')#" EQ "Y">
        ...
        function enableInputTime()
        {
            <cfif "#getblockid.recordcount#" NEQ 0>                
                <cfif "#getblockid.ot_type_block#" EQ "Y">
                    $(".inputOT_type").prop("disabled", false);    
                </cfif>
                
                <cfif "#getblockid.day_type_block#" EQ "Y">
                    $(".inputDayType").prop("disabled", false); 
                </cfif>
        ...
        function alertcheck() {
            ...
            title: 'Leave check before timesheet submission',
            message: "<p>Have you submitted all your leaves related to this timesheet and have all the leaves been approved?</p>"+
            "<p>Kindly get all your leaves approved before submitting your timesheet.</p>",
            buttons: {
                cancel: {
                    label: "No, I have not.",
                    className: 'btn-no',
                    callback: function(){
                        ...
                        $.notify({
                            title: '<strong>Submit your leave applications first!</strong><br /><br />',
                            message: 'Please make sure that all your leaves related to this timesheet are submitted and approved by'
                                    +' your respective managers before submitting your timesheet.'
                                    +'<br><br>Your approved leaves will automatically flow to your timesheet',
                            target: '_blank'
                            },{
                                // settings
                                element: 'body',
                ...
                noclose: {
                    label: "I do not have any leave related to this timesheet.",
                    className: 'btn-na',
                    callback: function(){
                        enableInputTime();document.getElementById('form1').submit();
                        return true;
                    }
                },
                ok: {
                    label: "Yes, I have submitted my leave.",
                    className: 'btn-yes',
                    callback: function(){
                        enableInputTime();document.getElementById('form1').submit();
                    }
        ...
        function calot(dayvar)
        {
            ...
            var ot_type = document.getElementById('ottype'+dayvar).value;
            ...
            <cfloop query="ot_calc">
                if(ot_type == '#ot_calc.sizeid#')
                {
                    <cfloop list="#daylist#" delimiters="," index="dd">
                        <cfif #dd# EQ "WD"> 
                            if (daytype == '')
                        <cfelse>
                            else if (daytype == '#dd#')
                        </cfif>
                        {
                            <cfloop from="1" to="8" index="ee">
                                <cfif val(evaluate('ot_calc.#dd#OT#ee#')) neq 0>
                                    othourpoint = #val(evaluate('ot_calc.#dd#OT#ee#'))#;
                                    <cfbreak>
                                </cfif>
                            </cfloop>
                            
                            <cfloop from="1" to="8" index="ee">
                                <cfif val(evaluate('ot_calc.#dd#OT#ee#')) EQ -1>
                                    othourpoint = #val(evaluate('ot_calc.#dd#OT#ee#'))#;
                                    <cfbreak>
                                </cfif>
                            </cfloop>
                            minothourpoint = #val(evaluate('ot_calc.#dd#MINOT'))#;
                            maxothourpoint = #val(evaluate('ot_calc.#dd#MAXOT'))#;
                            roundothourpoint = #val(evaluate('ot_calc.#dd#ROUNDUP'))#;
                        }
                    </cfloop>
                }
                <cfif otcount LT "#ot_calc.recordcount#">
                    else <cfset otcount += 1>
                </cfif>
            </cfloop>
            ...
            else
            {
                if(parseFloat(othourpoint) != 0 )
                {
                    ...
                    if(hournow > parseFloat(othourpoint))                                               //total hour exceed normal working hour - eligible for ot
                    {
                        ...
                        if(minothourpoint != 0 || maxothourpoint != 0 || roundothourpoint != 0)         //either value will enter if statement
                        {
                            if(roundothourpoint != 0 && (otnow >= 0))                                   //round up first
                            {
                                if(roundothourpoint >= 1){                                              //round to the nearest point
                                    otnow = Math.round(parseFloat(otnow));
                                }
                                else if(roundothourpoint <= -1){                                        //round down
                                    otnow = Math.floor(parseFloat(otnow));
                                }   
...
.inputTime:disabled, .inputOT_type:disabled, .inputDayType:disabled, .savesubmit:disabled {
    opacity: 0.45;
    cursor: not-allowed;
}
...
<!----------------------------->
    ...
    <div id="Book1_8828" align=center x:publishsource="Excel">
        <form name="form1" id="form1" method="post" action="timesheetprocess.cfm">
            ...
            <table border=0 cellpadding=0 cellspacing=0 width=735 style='border-collapse:collapse;table-layout:fixed;width:800pt'>
                <col width=40 span=3 style='width:40pt'>
                <col width=45 span=2 style='width:45pt'>
                <col width=80 span=2 style='width:80pt'>
                <col width=80 span=1 style='width:80pt'>
                <col width=60 span=2 style='width:60pt'>
                <col width=50 style='width:50pt'>
                <col width=181 style='mso-width-source:userset;mso-width-alt:5741;width:181pt'>
                ...
                <tr height=20 style='height:15.0pt'>
                    <td colspan=2 rowspan=3 height=60 class=xl1188828 style='border-right:1pt solid black;border-bottom:1pt solid black;height:45.0pt'>Client</td>
                    <td colspan=7 class=xl1248828 width=384 style='border-left:none;width:288pt'>Company Name &amp; Address</td>
                    <td colspan=9 class=xl898828 width=221 style='width:166pt'>Contact Person &amp; Tel</td>
                    <td class=xl158828></td>
                    <td class=xl158828></td>
                </tr>
                
                <tr height=20 style='height:15.0pt'>
                    <td colspan=7 height=20 class=xl1028828 style='height:15.0pt;border-left:none'>#getplacementlist.custname#</td>
                ...
                <tr height=20 style='height:15.0pt'>
                    <td colspan=7 height=20 class=xl1028828 style='height:15.0pt;border-left:none'>
                        #getcustdetail.add1# #getcustdetail.add2#
                        <cfif getcustdetail.add3 neq "">
                            <br>
                        </cfif>
                        #getcustdetail.add3# #getcustdetail.add4#
                    </td>
                ...
                <tr height=21 style='mso-height-source:userset;height:15.75pt'>
                    <td colspan=2 rowspan=2 height=41 class=xl1188828 style='border-right:1pt solid black;border-bottom:1pt solid black;height:30.75pt'>Staff</td>
                    <td colspan=7 class=xl1248828 width=320 style='border-right:1pt solid black;border-left:none;width:240pt'>Name</td>
                    <td colspan=9 class=xl1248828 width=128 style='border-right:1pt solid black;border-left:none;width:96pt'>NRIC</td>
                </tr>
                ...
                <cfquery name="getAllLeave" datasource="#dts#">
                ...
                <tr height=20 style='height:15.0pt'>
                    <td colspan=7 height=20 class=xl1258828 style='border-right:1pt solid black;height:15.0pt;border-left:none'>#getplacementlist.empname#</td>
                    <td colspan=9 class=xl1288828 width=128 style='border-left:none;width:96pt'>#emp_data.nricn#</td>
                    <td rowspan="3" height=20 class=xl748828 style='height:15.0pt;border-top:none;display: none' colspan="2">
                        <input type="hidden" name="enddate" id="enddate" value="<cfif gettimesheet.recordcount neq 0>#dateformat(gettimesheet.assigndate,'dd/mm/yyyy')#</cfif> " size="12" readonly><!--- <img src="/images/cal.gif" width=17 height=15 border=0 onClick="showCalendarControl(document.getElementById('enddate'));"> --->
                    </td>
                </tr>
 
                    
                <tr height=20 style='height:15.0pt'>
                    <td colspan=18 rowspan=2 height=40 class=xl1118828 width=576 style='height:30.0pt;width:432pt'>
                        I hereby certify that the hours shown below were rendered
                        by me during the specific dates and are certified as being correct by an
                        authorized representative of the abovenamed Client.
                ...
                <tr height=20 style='height:15.0pt'>
                    ...
                    <td rowspan=2 class=xl1078828 width=64 style='border-top:none;width:48pt'>Normal Hours Worked</td>
                    <td class=xl1078828 style='border-top:none'>&nbsp;</td>
                    ...
                    <td class=xl738828 style='border-top:none; display:none'>&nbsp;</td>
                    <td class=xl738828 style='border-top:none; display:none'>&nbsp;</td>
                    <td class=xl1078828 style='border-top:none;border-left:none;width:118;'>&nbsp;</td>
                    <td class=xl1078828></td>
                    <td class=xl1078828 style='border-top:none;border-left:none;width:118pt;' colspan="5"></td>
                </tr>
                <tr height=32 style='height:24.0pt'>
                    ...
                    <td class=xl858828 width=64 style='border-top:none;border-left:none;width:48pt'>Date</td>
                    <td colspan="2" class=xl838828 width=100 style='border-top:none;border-left:none;width:96pt'>Day Type<br>(Full Day/AM/PM)</td>
                    ...
                    <td class=xl838828 width=36 style='border-top:none;width:24pt'>OT Hours Worked</td>
                    ...
                    <td class=xl838828 width=157 style='border-top:none;border-left:none;width:118pt' colspan="7">Remarks</td>
                    <td class=xl158828></td>
                    <td class=xl158828></td>
                </tr>
                <tr height=32 style='height:24.0pt'>
                    ...
                    <td class=xl718828 width=64 style='border-top:none;border-left:none;width:48pt'>&nbsp;</td>
                    <td class=xl858828 width=64 style='border-top:none;border-left:none;width:48pt'></td>
                    <td class=xl858828 width=64 style='border-top:none;border-left:none;width:48pt'>hh:mm</td>
                    <td class=xl858828 width=64 style='border-top:none;border-left:none;width:48pt'>hh:mm</td>
                    <td class=xl858828 width=64 style='border-top:none;border-left:none;width:48pt'>hh:mm</td>
                    <td class=xl858828 width=64 style='border-top:none;border-left:none;width:48pt'>(less break)</td>
                    ...
                    <td class=xl858828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>1.0</td>
                    <td class=xl858828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>2.0</td>
                    <td class=xl718828 width=157 style='border-top:none;border-left:none;width:118pt' colspan="7">&nbsp;</td>
                    <td class=xl158828></td>
                    <td class=xl158828></td>
                </tr>
                <tr height=20 style='height:15.0pt'>
                    <td colspan=18 height=20 class=xl1298828 style='border-right:1pt solid black;border-left:1pt solid black;height:15.0pt'>
                        <cfif month(startdate) eq month(enddate)>
                            #dateformat(startdate,'Mmm yyyy')#
                        <cfelse>
                            #dateformat(startdate,'Mmm yyyy')# to #dateformat(enddate,'Mmm yyyy')#
                        </cfif>
                    </td>
                    <td class=xl158828></td>
                    <td class=xl158828></td>
                ...
                <cfloop from="0" to="#nonscount#" index="i">
                    ...
                    <cfquery name="GetSpecificTime" dbtype="query">
                        ...
                        and (placementno = '#getplacementlist.placementno#')
                    ...
                    <cfif #GetSpecificTime.recordcount# neq 0> <!---if data found for that specific date, replace it instead of using default value--->
                        ...
                        <tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='f3bd90';" height=20 
                            style='height:15.0pt'>
                            <td height=20 class=xl948828 style='height:15.0pt;border-top:none'>
                                <cfif startmonth neq dateformat(currentdate,'Mmm - yyyy')>
                                    #dateformat(currentdate,'Mmm')#<cfset startmonth = dateformat(currentdate,'Mmm - yyyy')>
                                </cfif>
                            </td>
                            ...
                            <td class=xl768828 style='border-top:none;border-left:none'>
                                ...
                                <select class="inputDayType" name="leavetype#i#" id="leavetype#i#" 
                            ...
                            <td class=xl768828 style='border-top:none;border-left:none'>
                                <select class="inputDayType" name="startampm#i#" id="startampm#i#" <cfif GetSpecificTime.stcol eq "" or GetSpecificTime.stcol eq "PH" or GetSpecificTime.stcol eq "OD" or GetSpecificTime.stcol eq "RD"> style="display:none"</cfif>>
                            ...
                            <td class=xl768828 style='border-top:none;border-left:none'>
                                <select class="inputOT_type" name="ottype#i#" id="ottype#i#" onChange="makehour('#i#');" style="width: 80px;">
                                    <cfloop query="ot_calc">
                                        <option value="#ot_calc.sizeid#" <cfif "#GetSpecificTime.ot_type#" EQ "#ot_calc.sizeid#">SELECTED</cfif>>
                                            #ot_calc.desp#
                                        </option>
                                    </cfloop>
                                </select>
                            ...
                            <td class=xl778828 width=66 style='border-top:none;border-left:none;width:65pt;text-align:center;' nowrap> 
                            ...
                            <td class=xl778828 width=64 style='border-top:none;border-left:none;width:65pt;text-align:center' nowrap>
                            ...
                            <td class=xl778828 width=64 style='border-top:none;border-left:none;width:48pt;text-align:center;'>
                                <select class="inputTime" name="break#i#" id="break#i#" onChange="makehour('#i#')">
                                    ...
                                    <cfloop from="0" to="120" index="a" step="15">
                                        <option value="#a/60#" <cfif GetSpecificTime.breaktime eq a/60>selected</cfif>>#timeformat(dateadd('n',a,timenow),'HH:MM')#</option>
                                    </cfloop>
                                </select>
                            </td>
                    
                            <td class=xl788828 width=64 style='border-top:none;border-left:none;width:48pt;text-align: center;'>
                            ...
                            <td class=xl798828 width=40 style='border-top:none;border-left:none; width:40pt;text-align: center'>
                                <input class="inputOT" type="number" name="ot#i#" id="ot#i#" value="#numberformat(GetSpecificTime.othour,'.__')#" size="4" 
                                       onKeyUp="ottotal();" onChange="ottotal();" style="width:55px;text-align: center" step="0.01">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="ot15#i#" id="ot15#i#" value="#numberformat(GetSpecificTime.ot15hour,'.__')#" size="3" onKeyUp="ot15total();" style="width:32px" >
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="ot20#i#" id="ot20#i#" value="#numberformat(GetSpecificTime.ot2hour,'.__')#" size="3" onKeyUp="ot2total();" style="width:32px" >
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="ot30#i#" id="ot30#i#" value="#numberformat(GetSpecificTime.ot3hour,'.__')#" size="3" onKeyUp="ot3total();" style="width:32px" >
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otrd1#i#" id="otrd1#i#" value="#numberformat(GetSpecificTime.otrd1hour,'.__')#" size="3" onKeyUp="otrd1total();" style="width:32px" >
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otrd2#i#" id="otrd2#i#" value="#numberformat(GetSpecificTime.otrd2hour,'.__')#" size="3" onKeyUp="otrd2total();" style="width:32px" >
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otph1#i#" id="otph1#i#" value="#numberformat(GetSpecificTime.otph1hour,'.__')#" size="3" onKeyUp="otph1total();" style="width:32px" >
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otph2#i#" id="otph2#i#" value="#numberformat(GetSpecificTime.otph2hour,'.__')#" size="3" onKeyUp="otph2total();" style="width:32px" >
                            </td>
                            ...
                            <td class=xl758828 style='border-top:none;border-left:none;text-align: center;' colspan="7">
                                <input type="text" name="remarks#i#" id="remarks#i#" value="#GetSpecificTime.remarks#" size="29" maxlength="450">
                            </td>
                    ...
                    <cfelse> <!---data not found, replace it with default value--->
                        ...
                        <cfquery name="getLeave" dbtype="query">
                            SELECT * from getAllLeave
                            WHERE placementno = '#form.pno#'
                            AND status = 'APPROVED'
                        ...
                        <tr onMouseOut="javascript:this.style.backgroundColor='';" onMouseOver="javascript:this.style.backgroundColor='f3bd90';" height=20 
                            ...
                            <td height=20 class=xl948828 style='height:15.0pt;border-top:none'>
                                <cfif startmonth neq dateformat(currentdate,'Mmm - yyyy')>
                                    #dateformat(currentdate,'Mmm - yy')#<cfset startmonth = dateformat(currentdate,'Mmm - yyyy')>
                                </cfif>
                            </td>
                            ...
                            <td class=xl768828 style='border-top:none;border-left:none'> 
                                <select class="inputDayType" name="startampm#i#" id="startampm#i#" <cfif #getLeave.recordcount# neq 0 AND #getLeave.leavetype# neq 'WD' AND #getLeave.leavetype# neq 'OD' AND #getLeave.leavetype# neq 'RD' AND #getLeave.leavetype# neq 'PH'> style="display:block" <cfelse>style="display:none" </cfif> >
                            ...
                            <td class=xl768828 style='border-top:none;border-left:none'>
                                <select class="inputOT_type" name="ottype#i#" id="ottype#i#" onChange="makehour('#i#');" style="width: 80px">
                                    <cfloop query="ot_calc">
                                        <option value="#ot_calc.sizeid#">#ot_calc.desp#</option>
                                    </cfloop>
                                </select>            
                            </td>
                            ...
                            <td class=xl778828 width=64 style='border-top:none;border-left:none;width:65pt;text-align:center;' nowrap>
                                ...
                                <select class="inputTime" name="hre#i#" id="hre#i#" onChange="makehour('#i#');">
                                    <cfloop from="0" to="23" index="aaa">
                                        <option value="#numberformat(aaa,'00')#" <cfif hre eq numberformat(aaa,'00')>Selected</cfif>>
                                            #numberformat(aaa,'00')#</option>
                                        </cfloop>
                                </select>
                                <select class="inputTime" name="minae#i#" id="minae#i#" onChange="makehour('#i#');">
                                    <cfloop from="0" to="59" index="aaa">
                                        <option value="#numberformat(aaa,'00')#" <cfif minae eq numberformat(aaa,'00')>Selected</cfif>>
                                            #numberformat(aaa,'00')#
                                        </option>
                                    </cfloop>
                                </select>
                            ...
                            <td class=xl788828 width=64 style='border-top:none;border-left:none;width:48pt;text-align: center'>
                                <input type="text" name="totalhour#i#" id="totalhour#i#" value="<cfif pholiday neq 1 >#numberformat(evaluate('getottable.workh#DayOfWeek(currentdate)#'),'.__')#<cfelse>0.00</cfif>" size="4" readonly style="text-align: center">
                            </td>

                            <cfset totalhour = totalhour + numberformat(evaluate('getottable.workh#DayOfWeek(currentdate)#'),'.__')>

                            <td class=xl798828 width=40 style='border-top:none;border-left:none; width:50pt;text-align: center'>
                                <input class="inputOT" type="number" name="ot#i#" id="ot#i#" value="0.00" size="4"  onKeyUp="ottotal();" onChange="ottotal()"
                                       style="width:55px;text-align: center" step="0.01">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="ot15#i#" id="ot15#i#" value="0.00" size="3"  onKeyUp="ot15total();" style="width:32px">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="ot20#i#" id="ot20#i#" value="0.00" size="3"  onKeyUp="ot2total();" style="width:32px">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="ot30#i#" id="ot30#i#" value="0.00" size="3"  onKeyUp="ot3total();" style="width:32px">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otrd1#i#" id="otrd1#i#" value="0.00" size="3"  onKeyUp="otrd1total();" style="width:32px">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otrd2#i#" id="otrd2#i#" value="0.00" size="3"  onKeyUp="otrd2total();" style="width:32px">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otph1#i#" id="otph1#i#" value="0.00" size="3"  onKeyUp="otph1total();" style="width:32px">
                            </td>
                            <td class=xl798828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                                <input type="text" name="otph2#i#" id="otph2#i#" value="0.00" size="3"  onKeyUp="otph2total();" style="width:32px">
                            </td>
                            ...
                            <td class=xl758828 style='border-top:none;border-left:none;text-align: center' colspan="5">
                                <input type="text" name="remarks#i#" id="remarks#i#" <cfif pholiday eq "1">value="#phdesp#"</cfif> maxlength="450"  size="29">
                            </td>
                            <td class=xl158828></td>
                            <td class=xl158828></td>
                        </tr>
                ...
                <!---recalculate OT based on geblockid table, [20180430, Alvin]--->
                <cfif "#getblockid.recordcount#" NEQ 0 AND "#getblockid.autoOT#" EQ "Y">
                    <script type="application/javascript">
                        setTimeout(function(){  
                ...
                <tr>
                    <td colspan=9 height=20 class=xl1158828 width=448 
                        style='border-right:1pt solid black;border-left:1pt solid black; border-bottom:1pt solid black;'>
                        Total hours<!---  for the period (exclude breaks) --->
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt;text-align: center'>
                        <input type="text" name="totalhour" id="totalhour" value="#numberformat(totalhour,'.__')#" size="4" style="text-align: center" readonly>
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt;text-align: center'>
                        <input type="text" name="totalot" id="totalot" style="width:55px;padding-left: 5px" value="#numberformat(totalot,'.__')#" size="4" readonly>
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                        <input type="text" name="totalot15" id="totalot15" style="width:32px" value="#numberformat(totalot,'.__')#" size="3">
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                        <input type="text" name="totalot2" id="totalot2" style="width:32px" value="#numberformat(totalot2,'.__')#" size="3">
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                        <input type="text" name="totalot3" id="totalot3" style="width:32px" value="#numberformat(totalot3,'.__')#" size="3">
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                        <input type="text" name="totalotrd1" id="totalotrd1" style="width:32px" value="#numberformat(totalotrd1,'.__')#" size="3">
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                        <input type="text" name="totalotrd2" id="totalotrd2" style="width:32px" value="#numberformat(totalotrd2,'.__')#" size="3">
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                        <input type="text" name="totalotph1" id="totalotph1" style="width:32px" value="#numberformat(totalotph1,'.__')#" size="3">
                    </td>
                    <td class=xl808828 width=36 style='border-top:none;border-left:none; width:26pt; display:none'>
                        <input type="text" name="totalotph2" id="totalotph2" style="width:32px" value="#numberformat(totalotph2,'.__')#" size="3">
                    </td>
                    <td class=xl708828 width=157 style='border-top:none;border-left:none;width:118pt'>&nbsp;</td>
                    <td class=xl158828></td>
                    <td class=xl158828></td>
                </tr>
                ...
                <cfif #gettimesheet.recordcount# neq val('#nonscount#')>
                    <cfloop query="gettimesheet">
                        <cfif #gettimesheet.status# eq 'Submitted For Approval'>
                            <cfset submittedCount += 1>
                        </cfif>
                        <cfif #gettimesheet.editable# eq 'N'>
                ...
                <tr>
                    <td class=xl158828 align="center" colspan="18">
                        <input class="savesubmit" type="button" name="save" id="save" value="Save" onClick="
                            <cfif (#submittedStatus# eq True) OR (#editableStatus# eq TRUE)>
                               alert('Timesheet Has Been Submitted, No Update is Allowed')
                            <cfelse>
                               enableInputTime();savebtn();this.disabled = true;
                            </cfif>">
                        <input class="savesubmit" type="button" name="sub_btn" id="sub_btn" value="Submit" onClick="
                            <cfif (#submittedStatus# eq True) OR (#editableStatus# eq TRUE)>
                                alert('Timesheet Has Been Submitted, No Update is Allowed')
                            <cfelse>
                                alertcheck();

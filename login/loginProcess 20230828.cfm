...
<cflogin >
	...
	<cfif isdefined("form.userId") is false or isdefined("form.userPwd") is false>
		<!---added to direct to reset password page instead, [20170927, Alvin]--->
        <cfif #CGI.script_name# EQ '/reset/resetpassword.cfm'>
	        <cfinclude template="/reset/resetpassword.cfm">
	        <cfabort>
	    <cfelseif #CGI.script_name# EQ '/reset/resetpasswordprocess.cfm'>
	        <cfinclude template="/reset/resetpasswordprocess.cfm">
	        <cfabort>
	    <cfelseif #CGI.script_name# EQ '/reset/resetsuccess.cfm'>
	        <cfinclude template="/reset/resetsuccess.cfm">
	        <cfabort>
	    <cfelse>
    ...
    <cfif isdefined('form.profile') eq false>
    ...
    <cfif validate_company_id.recordcount neq 0 and validate_company_id.userDSN neq "">
		<cfquery name="get_DB_DATA" datasource="payroll_main">
			SELECT DS_NAME FROM payroll_dscontrol WHERE CompID=<cfqueryparam cfsqltype="cf_sql_char" value="#form.companyid#">
		</cfquery>
		...
		<cfquery name="validateEmp" datasource="#get_DB_DATA.DS_NAME#">
			...
			ON e.empno=p.empno
			WHERE 
			UserName=<cfqueryparam cfsqltype="cf_sql_char" value="#form.userid#">
			AND UserPass=<cfqueryparam cfsqltype="cf_sql_char" value="#form.userpwd#">
			AND CompanyID=<cfqueryparam cfsqltype="cf_sql_char" value="#form.companyid#">
			AND p.paystatus ="A"
			<!--- AND status = '1'; --->
		</cfquery>
    ...
    <cfif isdefined('form.profile')>
        <cfif form.submit eq 'Associate'>
            <cfset checkprofile = form.submit>
            <cfparam name="session.checkprofile" default="">
        </cfif> 
    </cfif>
	
    <cfif validateUser.Recordcount eq 1 and validateEmp.Recordcount eq 1 and isdefined('form.profile') eq false>
        <cfinclude template="chooseprofile.cfm">
        <cfabort>
	<cfelseif validateUser.Recordcount eq 1 and validlogin eq 1 and isdefined('checkprofile') eq false>
		...
		<cfquery name="update_user_last_login" datasource="payroll_main">
			...
			AND userpwd=<cfqueryparam cfsqltype="cf_sql_char" value="#form.userpwd#">
			AND userCmpID=<cfqueryparam cfsqltype="cf_sql_char" value="#form.companyid#">
		</cfquery>

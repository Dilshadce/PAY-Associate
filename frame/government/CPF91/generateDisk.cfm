...
<cfset filedir = filenewdir&"file"&huserid&".dtl">
...
SELECT a.empno as empno FROM pmast AS a LEFT JOIN pay_tm AS b ON a.empno=b.empno
			WHERE epfcat = #cat# and a.paystatus = "A" AND b.payyes = "Y"
</cfquery>

<cfquery name="emplistnew" datasource="#dts#">
SELECT empno FROM pmast WHERE epfcat = #cat# and paystatus = "A"
</cfquery>
...
<cfquery name="sumFWLevy_qry" datasource="#dts#">
	select sum(levy_fw_w) as fwlevy from comm as c where empno in
		(<cfqueryparam cfsqltype="cf_sql_varchar" value="#valuelist(emplist.empno)#" list="yes" separator=",">) 
	and levy_fw_w <> ''
</cfquery>
...
<cfelse>
	<cfquery name="get_count_fwl" datasource="#dts#">
		select levy_fw_w as get_FWL from comm where empno in
		(<cfqueryparam cfsqltype="cf_sql_varchar" value="#valuelist(emplist.empno)#" list="yes" separator=",">)
		and levy_fw_w <> ''
	</cfquery>
...
<cfloop query="get_count_fwl">
	<cfif val(get_count_fwl.get_FWL) gt 0>
...
FROM comm WHERE empno in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#valuelist(emplistnew.empno)#" list="yes" separator=",">)
</cfquery>

<cfquery name="get_count_sdl" datasource="#dts#">
SELECT levy_sd
FROM comm 
WHERE empno in (<cfqueryparam cfsqltype="cf_sql_varchar" value="#valuelist(emplistnew.empno)#" list="yes" separator=",">)
</cfquery>
<cfset sdl_count = 0>
...
<cfset cpf_no = trim(getContent_qry.epfno) >
...
<cfif val(total_cpf_penalty)/100 gt 0>
<cfset total_empcc_contrib = total_empcc_contrib + val(total_cpf_penalty)/100 >
</cfif>
<cfif val(total_fwl_penalty)/100 gt 0>
<cfset total_empcc_contrib = total_empcc_contrib + val(total_fwl_penalty)/100 >
</cfif>
...
<cfset yourFileName2="#filename#.DTL">

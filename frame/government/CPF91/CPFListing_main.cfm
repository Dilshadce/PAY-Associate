...
<div class="mainTitle">CPF91 CPF Lising</div>
<div class="tabber">
<form name="gForm" action="CPFListing_rep<cfif left(dts,4) eq 'beps'>beps</cfif>.cfm" method="post" target="_blank">
<table class="form">
	<tr>
		<th>Report Format</th>
		<td>
	  ...
	  for(var i=0;i<output.length;i++) {
		if(output[i].text.toLowerCase().indexOf(input)>=0 && i != 0){
		  output[i].selected=true;
		   if(fieldid == "empno")
		  {
			  document.getElementById('empno1').options[i].selected=true;
		  }
		  break;
		  }
...
<cfoutput>
	...
	<th>Employee No.</th>
	<td>
	<select name="empno" id="empno" onChange="document.getElementById('empno1').selectedIndex=this.selectedIndex;">

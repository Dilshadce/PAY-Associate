...
<cfset currentFile=getToken(cgi.PATH_INFO,listlen(request.path_info,"/"),"/")>
<!--- <cfset currentFile="http://localhost/government/CPF91/CPFRefund_upd.cfm"> --->
...
<body>
<div class="mainTitle">Update CPF Refund</div>
<div class="tabber">
<cfoutput>
			...
			<span class="ttype">Search By :
				<select name="sType">
			      ...
			      <option value="plineno">CPF No.</option>
			    </select>
				Search for : 
		...
		<cfform name="form1" action="#currentFile#?list=" method="post" onsubmit="return checkValue();">
				...
				<cfif getData.recordcount>
					...
					<tr>
						...
						<td>
	           			...
	           			<td width="">
		           			<a href="##" onclick="window.open ('CPFRefund_updForm.cfm?empno=#getData.empno#','windowname1', 'width=900, height=700, left=30, top=50');"><img height="18px" width="18px" src="/images/edit.ICO" alt="Edit" border="0">Update</a>
		           			<!--- <input type="button" name="update" value="Update" onclick="window.open ('CPFRefund_updForm.cfm','windowname1', 'width=900, height=700, left=30, top=50');"> --->
						</td>
					</tr>
					</cfoutput>
				<cfelse>
			...
			<!--- <center>
				<input type="button" name="exit" value="Exit" onclick="window.location.href='CPFRefund_main.cfm';">
			</center> --->
		</cfform>
		<cfoutput>
			<cfif total_pages gt 1>
		...
		<div><br /><a href="/government/CPF91/CPFRefund_main.cfm"><strong>< Back</strong></a></div>
</div>
</body>
</html>	
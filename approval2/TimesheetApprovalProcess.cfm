...
<cfif ("#IsDefined('form.checkerlist')#" AND "#form.checkerlist#" NEQ "") AND ("#IsDefined('form.tsstatus')#" AND "#form.tsstatus#" NEQ "")>
    ...
    <cfloop list="#form.checkerlist#" delimiters="," index="a">
        <cfset pno = "#ListGetAt(a, 1, '_')#">
        <cfset tsmonth = "#ListGetAt(a, 2, '_')#">
        <cfset tsyear = "#ListGetAt(a, 3, '_')#">
        ...
        <cfloop query="getts">  
            <cfif getHQstatus.ApproveVerify eq 'VERIFIED'>
                <cfif trim(Evaluate('form.remarks_#a#')) eq ''>
                    <cfset 'form.remarks_#a#' = "Verified">
                </cfif>
            </cfif>            
            ...
            <cfquery name="updateTS" datasource="#dts#">
                ...
                updated_on = now(),
                updated_by = "#HUserID#",
                MGMTREMARKS = "#Evaluate('form.remarks_#a#')#",
    ...
    <cfoutput>
        <script>
            alert("#Listlen(form.checkerlist, ',')# Timesheet #LCase(timestatus)#!");
...
<cfelse>
    ...
    <cfset pno = "#ListGetAt(url.updateDetails, 1, '_')#">
    <cfset tsmonth = "#ListGetAt(url.updateDetails, 2, '_')#">
    <cfset tsyear = "#ListGetAt(url.updateDetails, 3, '_')#">
    ...
    <cfloop query="getts">
        <cfif getHQstatus.ApproveVerify eq 'VERIFIED'>
            <cfif TRIM(URLDECODE(url.mgmtremarks)) eq ''>
                <cfset url.mgmtremarks = "Verified">
            </cfif>
        </cfif>
    ...
    <cfoutput>
        <script>
            alert("Timesheet #LCase(timestatus)#!");
            window.open('/approval2/TimesheetApprovalMain.cfm', '_self');
        </script>
    </cfoutput>
</cfif>
...
<cffunction name="emailNotification">
    ...
    <cfif #getemail.setting2# eq 'Y'>
        <cfif getdata.validemail neq "" AND #IsValid("email", "#trim(getdata.validemail)#")#>
